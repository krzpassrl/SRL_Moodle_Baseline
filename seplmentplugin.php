...
/**
 * This file contains the functions for sepl_plugin abstract class
 *
 *
 * @package   mod_sepl
...
require_once('/var/www/html/moodle/mod/sepl/sphereengine/autoload.php');

/**
 * Abstract class for sepl_plugin (submission/feedback).
 *
 * @package   mod_sepl
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class sepl_plugin {

    /** @var sepl $seplment the seplment record that contains the global
     *              settings for this sepl instance
     */
    protected $seplment;
    /** @var string $type seplment plugin type */
    private $type = '';
     ...
     * @param string $type
     */
    public final function __construct(sepl $seplment, $type) {
     ...
     * form for display in the settings page for the seplment.
    ...
    /**
     * The seplment subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'seplsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - sepl_submission or sepl_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this seplment.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/sepl/' . substr($this->get_subtype(), strlen('sepl')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('seplment'=>$this->seplment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('sepl_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('sepl_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->seplment = $this->seplment->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('sepl_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($seplment) {
                $dbparams = array('seplment'=>$seplment->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
    ...
    /**
     * Given a field name, should return the format of an or field that is part of
     * this plugin. This is used when exporting to portfolio.
     *
     * @param string $name Name of the field.
     ...
     * and version.
     *
     * @param string $type The old seplment subtype
     * @param int $version The old seplment version
    ...
    /**
     * Upgrade the settings from the old seplment to the new one
     *
     * @param context $oldcontext The context for the old seplment module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldseplment, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_sepl', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade sepl_submission or sepl_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_sepl', $params);
        return false;
    }
    ...
    /**
     * The seplment has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this seplment plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade sepl_submission or sepl_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if (!($storedfile = $fs->get_file($this->seplment->get_context()->id,
...
//$aaa = 'test123';
//var_dump($aaa);
$testClient = $client->submissions->create('SEPLTEST_123', 'int main() {};', 11);#create('int main() {}', 11, 'Robsontest');/
    ...
    /**
     * This allows a plugin to render a page in the context of the seplment
     *
     * If the plugin creates a link to the seplment view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=seplfeedback|seplsubmission
     ...
     * right below the activity's "intro" section on the main seplment page.
     *
     * @return string
     */
    public function view_header() {
#$ch = curl_init('http://api.compilers.sphere-engine.com/api/v3/languages?access_token=02994435062fb95b5f3e6938e5aa433e');
#$data = array('SphereEngine-Curl/1.0');
$token = "dadeea82cdd4b840cbde18858d9f17d6fb11a957";
/*$params = array('access_token'=>'dadeea82cdd4b840cbde18858d9f17d6fb11a957'
             );
$endpoint = 'http://api.compilers.sphere-engine.com/api/v3/languages';#?access_token=dadeea82cdd4b840cbde18858d9f17d6fb11a957';
#$params
...
$out = html_writer::select($jsonString,'Compiler','');
...
$jsonStr = json_decode($server_output2, true);
#var_dump($jsonStr);
$out2 = html_writer::select($jsonStr,'Problems','');
...
#var_dump($out2);
     ...
     * enabled/disabled per seplment instance.
     *
     * @return bool
     */
