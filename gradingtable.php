...
/**
 * Extends table_sql to provide a table of seplment submissions
 *
 * @package   mod_sepl
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_grading_table extends table_sql implements renderable {
    /** @var sepl $seplment */
    private $seplment = null;
    /** @var int $perpage */
    private $perpage = 10;
     ...
     */
    public function __construct(sepl $seplment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        $this->hasgrantextension = has_capability('mod/sepl:grantextension',
        ...
        $this->hasviewblind = has_capability('mod/sepl:viewblinddetails',
                $this->seplment->get_context());
        ...
        if ($this->seplment->get_instance()->markingworkflow &&
            $this->seplment->get_instance()->markingallocation) {
            ...
            if (!empty($workflowstates)) {
                ...
                if ($workflowfilter == ASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('sepl_user_flags', 'workflowstate', true, true).')';
        ...
        if ($this->seplment->get_instance()->markingworkflow &&
            $this->seplment->get_instance()->markingallocation &&
            has_capability('mod/sepl:manageallocations', $this->seplment->get_context())) {
        ...
        $this->gradinginfo = grade_get_grades($this->seplment->get_course()->id,
                                              'mod',
                                              'sepl',
                                              $this->seplment->get_instance()->id,
                                              $users);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this seplment.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'sepl') .
               $this->seplment->get_uniqueid_for_user($row->userid);
    }
    ...
    public function col_workflowstate($row) {
        ...
        return get_string('markingworkflowstate' . $state, 'sepl');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($this->quickgrading && has_capability('mod/sepl:manageallocations', $this->seplment->get_context()) &&
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->seplment->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->seplment->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->seplment->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->seplment->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            return $group->name;
        } else if ($this->seplment->get_instance()->preventsubmissionnotingroup) {
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $fullname = $this->output->action_link($link, $this->seplment->fullname($row));
        } else {
            $fullname = $this->seplment->fullname($row);
        }

        if (!$this->seplment->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'sepl', $this->seplment->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this seplment.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->seplment->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'sepl', $name),
                                            'mod_sepl');
            $urlparams = array('id' => $this->seplment->get_course_module()->id,
                               'rownum'=>$this->rownum,
                               'action'=>'grade');
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->seplment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $status, 'sepl'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'sepl');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'sepl'), 'submissiongraded');
                } else if (!$timesubmitted) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'sepl', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'sepl', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        $submissionsopen = $this->seplment->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        ...
        if ($this->seplment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    ...
                    !$this->seplment->get_instance()->submissiondrafts) {
        ...
        if (($this->seplment->get_instance()->duedate ||
                $this->seplment->get_instance()->cutoffdate) &&
     ...
     *
     * @param sepl_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(sepl_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('sepl')), 'sepl');
            ...
            if ($plugin->get_subtype() == 'seplsubmission') {
                if ($this->seplment->get_instance()->teamsubmission) {
                    ...
                    if ($row->status == ASSIGN_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->seplment->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->seplment = $this->seplment->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->seplment = $this->seplment->get_instance()->id;
    ...
    public function get_course_id() {
        return $this->seplment->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->seplment->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this seplment accept submissions
     */
    public function submissions_enabled() {
        return $this->seplment->is_any_submission_plugin_enabled();
    }
