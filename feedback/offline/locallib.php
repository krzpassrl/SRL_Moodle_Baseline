 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_feedback_offline extends sepl_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'seplfeedback_offline');
    }
    ...
    /**
     * Loop through uploaded grades and update the grades for this seplment
     *
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified) {
        ...
        $gradeimporter = new seplfeedback_offline_grade_importer($importid, $this->seplment);
        ...
        // Does this seplment use a scale?
        $scaleoptions = null;
        if ($this->seplment->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id'=>-($this->seplment->get_instance()->grade)))) {
                $scaleoptions = make_menu_from_list($scale->scale);
            }
        }
        // We may need to upgrade the gradebook comments after this update.
        $adminconfig = $this->seplment->get_admin_config();
        ...
        while ($record = $gradeimporter->next()) {
            ...
            $usergrade = $this->seplment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $record->grade) {
                ...
                $skip = true;
            } else if ($this->seplment->grading_disabled($record->user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($this->seplment->get_instance()->grade > -1) &&
                      (($record->grade < 0) || ($record->grade > $this->seplment->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }

            if (!$skip) {
                $grade = $this->seplment->get_user_grade($record->user->id, true);

                $grade->grade = $record->grade;
                $grade->grader = $USER->id;
                if ($this->seplment->update_grade($grade)) {
                ...
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        ...
                        if (($plugin->get_subtype() . '_' . $plugin->get_type()) == $gradebookplugin) {
                            ...
                            $this->seplment->update_grade($grade);
                        }
                    }
                }
            }
        ...
        $o .= $renderer->render(new sepl_header($this->seplment->get_instance(),
                                                  $this->seplment->get_context(),
                                                  false,
                                                  $this->seplment->get_course_module()->id,
                                                  get_string('importgrades', 'seplfeedback_offline')));
        $o .= $renderer->box(get_string('updatedgrades', 'seplfeedback_offline', $updatecount));
    ...
    public function upload_grades() {
        ...
        require_capability('mod/sepl:grade', $this->seplment->get_context());
        require_once($CFG->dirroot . '/mod/sepl/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/sepl/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/sepl/feedback/offline/importgradeslib.php');
        require_once($CFG->libdir . '/csvlib.class.php');

        $mform = new seplfeedback_offline_upload_grades_form(null,
        ...
        if ($mform->is_cancelled()) {
            ...
            $importid = csv_import_reader::get_new_iid('seplfeedback_offline');
            $gradeimporter = new seplfeedback_offline_grade_importer($importid, $this->seplment);
            ...
            $mform = new seplfeedback_offline_import_grades_form(null, array('seplment'=>$this->seplment,
                                                                       'csvdata'=>$csvdata,
            ...
            $o .= $renderer->render(new sepl_header($this->seplment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->seplment->get_course_module()->id,
                                                            get_string('confirmimport', 'seplfeedback_offline')));
            $o .= $renderer->render(new sepl_form('confirmimport', $mform));
            $o .= $renderer->render_footer();
        } else if ($confirm) {
            ...
            $o .= $renderer->render(new sepl_header($this->seplment->get_instance(),
                                                            ...
                                                            false,
                                                            $this->seplment->get_course_module()->id,
                                                            get_string('uploadgrades', 'seplfeedback_offline')));
            $o .= $renderer->render(new sepl_form('batchuploadfiles', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function download_grades() {
        ...
        $filename = clean_filename(get_string('offlinegradingworksheet', 'seplfeedback_offline') . '-' .
                                   $this->seplment->get_course()->shortname . '-' .
                                   $this->seplment->get_instance()->name . '-' .
                                   $groupname .
                                   $this->seplment->get_course_module()->id);
    ...
    public function get_grading_actions() {
        return array('uploadgrades'=>get_string('uploadgrades', 'seplfeedback_offline'),
                    'downloadgrades'=>get_string('downloadgrades', 'seplfeedback_offline'));
    }
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $gradingmanager = get_grading_manager($this->seplment->get_context(), 'mod_sepl', 'submissions');
