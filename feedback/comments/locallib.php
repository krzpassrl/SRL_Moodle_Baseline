 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_feedback_comments extends sepl_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'seplfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('seplfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'seplfeedback_comments');
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'seplfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('seplfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->seplment = $this->seplment->get_instance()->id;
                return $DB->insert_record('seplfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->seplfeedback_comments_commentinline));
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $mform->addElement('selectyesno',
                           'seplfeedback_comments_commentinline',
                           get_string('commentinline', 'seplfeedback_comments'));
        $mform->addHelpButton('seplfeedback_comments_commentinline', 'commentinline', 'seplfeedback_comments');
        $mform->setDefault('seplfeedback_comments_commentinline', $default);
        // Disable comment online if comment feedback plugin is disabled.
        $mform->disabledIf('seplfeedback_comments_commentinline', 'seplfeedback_comments_enabled', 'notchecked');
   }
    ...
    protected function convert_submission_text_to_feedback($submission, $data) {
        ...
        $data->seplfeedbackcomments_editor['text'] = $text;
        $data->seplfeedbackcomments_editor['format'] = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $submission = $this->seplment->get_user_submission($userid, false);
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->seplfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
            $data->seplfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
        } else {
        ...
        $mform->addElement('editor', 'seplfeedbackcomments_editor', $this->get_name(), null, null);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->seplfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->seplfeedbackcomments_editor['format'];
            return $DB->update_record('seplfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->seplfeedbackcomments_editor['text'];
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldseplment, & $log) {
        if ($oldseplment->seplmenttype == 'online') {
            $this->set_config('commentinline', $oldseplment->var1);
            return true;
        }
        return true;
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->seplment = $this->seplment->get_instance()->id;
        if (!$DB->insert_record('seplfeedback_comments', $feedbackcomments) > 0) {
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the seplment
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('seplfeedback_comments',
    ...
    public function get_external_parameters() {
        ...
        return array('seplfeedbackcomments_editor' => $editorstructure);
    }
