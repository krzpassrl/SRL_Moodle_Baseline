 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_feedback_editpdf extends sepl_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'seplfeedback_editpdf');
    }
     ...
     * @param bool $readonly
     * @return seplfeedback_editpdf_widget
     */
    public function get_widget($userid, $grade, $readonly) {
        ...
        $feedbackfile = document_services::get_feedback_document($this->seplment->get_instance()->id,
                                                                 $userid,
                                                                 $attempt);
        ...
        if ($files = $fs->get_area_files($syscontext->id,
            ...
            foreach ($files as $file) {
                ...
                if ($filename !== '.') {
                    ...
                    $existingfile = $fs->get_file($this->seplment->get_context()->id,
                                                  'seplfeedback_editpdf',
                                                  'stamps',
                                                  $grade->id,
                                                  '/',
                    ...
                    if (!$existingfile) {
                        ...
                        $newrecord->contextid = $this->seplment->get_context()->id;
        ...
        if ($files = $fs->get_area_files($this->seplment->get_context()->id,
                                         'seplfeedback_editpdf',
                                         'stamps',
                                         $grade->id,
                                         "filename",
            ...
            foreach ($files as $file) {
                $filename = $file->get_filename();
                if ($filename !== '.') {
                    $url = moodle_url::make_pluginfile_url($this->seplment->get_context()->id,
                                                   'seplfeedback_editpdf',
                                                   'stamps',
                                                   $grade->id,
                                                   '/',
        ...
        $pagetotal = document_services::page_number_for_attempt($this->seplment->get_instance()->id,
                $userid,
                $attempt,
                $readonly);

        $widget = new seplfeedback_editpdf_widget($this->seplment->get_instance()->id,
                                                    $userid,
                                                    $attempt,
                                                    $url,
                                                    $filename,
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $files = document_services::list_compatible_submission_files_for_attempt($this->seplment, $userid, $attempt);
        // Only show the editor if there was a compatible file submitted.
        if (count($files)) {

            $renderer = $PAGE->get_renderer('seplfeedback_editpdf');

            $widget = $this->get_widget($userid, $grade, false);

            $html = $renderer->render($widget);
            $mform->addElement('static', 'editpdf', get_string('editpdf', 'seplfeedback_editpdf'), $html);
            $mform->addHelpButton('editpdf', 'editpdf', 'seplfeedback_editpdf');
    ...
    public function view(stdClass $grade) {
        ...
        if (page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->seplment->render_area_files('seplfeedback_editpdf',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('seplfeedback_editpdf_annot', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('sepl_grades', array('seplment'=>$this->seplment->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('seplfeedback_editpdf_annot', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('seplfeedback_editpdf_cmnt', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
