...
require_once($CFG->dirroot.'/mod/sepl/feedback/editpdf/fpdi/fpdi.php');
...
class pdf extends \FPDI {
    ...
    public function get_image($pageno) {
        ...
        if ($generate) {
            ...
            if (!file_exists($imagefile)) {
                $fullerror = '<pre>'.get_string('command', 'seplfeedback_editpdf')."\n";
                $fullerror .= $command . "\n\n";
                $fullerror .= get_string('result', 'seplfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars($result) . "\n\n";
                $fullerror .= get_string('output', 'seplfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars(implode("\n",$output)) . '</pre>';
                throw new \moodle_exception('errorgenerateimage', 'seplfeedback_editpdf', '', $fullerror);
            }
        }
    ...
    public static function ensure_pdf_compatible(\stored_file $file) {
        ...
        $temparea = \make_temp_directory('seplfeedback_editpdf');
    ...
    public static function test_gs_path($generateimage = true) {
        ...
        $testfile = $CFG->dirroot.'/mod/sepl/feedback/editpdf/tests/fixtures/testgs.pdf';
        if (!file_exists($testfile)) {
            $ret->status = self::GSPATH_NOTESTFILE;
            return $ret;
        }

        $testimagefolder = \make_temp_directory('seplfeedback_editpdf_test');
