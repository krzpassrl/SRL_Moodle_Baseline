...
/**
 * This file contains the ingest manager for the seplfeedback_editpdf plugin
 *
...
namespace seplfeedback_editpdf;
...
class document_services {
    ...
    /**
     * This function will take an int or an seplment instance and
     * return an seplment instance. It is just for convenience.
     * @param int|\sepl $seplment
     * @return sepl
     */
    private static function get_seplment_from_param($seplment) {
        global $CFG;

        require_once($CFG->dirroot . '/mod/sepl/locallib.php');

        if (!is_object($seplment)) {
            $cm = \get_coursemodule_from_instance('sepl', $seplment, 0, false, MUST_EXIST);
     ...
     */
    private static function hash($seplment, $userid, $attemptnumber) {
        if (is_object($seplment)) {
            $seplmentid = $seplment->get_instance()->id;
        } else {
            $seplmentid = $seplment;
        }
        return sha1($seplmentid . '_' . $userid . '_' . $attemptnumber);
    }
     ...
     */
    public static function list_compatible_submission_files_for_attempt($seplment, $userid, $attemptnumber) {
        global $USER, $DB;

        $seplment = self::get_seplment_from_param($seplment);
        ...
        if ($seplment->get_instance()->teamsubmission) {
            $submission = $seplment->get_group_submission($userid, 0, false);
        } else {
            $submission = $seplment->get_user_submission($userid, false);
        }
     ...
     * @return stored_file
     */
    public static function get_combined_pdf_for_attempt($seplment, $userid, $attemptnumber) {
        ...
        $grade = $seplment->get_user_grade($userid, true, $attemptnumber);
        if ($seplment->get_instance()->teamsubmission) {
            $submission = $seplment->get_group_submission($userid, 0, false);
        } else {
            $submission = $seplment->get_user_submission($userid, false);
        ...
        if (!$combinedpdf ||
            ...
            return self::generate_combined_pdf_for_attempt($seplment, $userid, $attemptnumber);
        }
        return $combinedpdf;
    }
     ...
     * @return stored_file
     */
    public static function generate_combined_pdf_for_attempt($seplment, $userid, $attemptnumber) {
        global $CFG;
        ...
        $files = self::list_compatible_submission_files_for_attempt($seplment, $userid, $attemptnumber);
        ...
        if ($files) {
            ...
            $tmpdir = \make_temp_directory('seplfeedback_editpdf/combined/' . self::hash($seplment, $userid, $attemptnumber));
        ...
        $record->contextid = $seplment->get_context()->id;
        $record->component = 'seplfeedback_editpdf';
     ...
     */
    public static function page_number_for_attempt($seplment, $userid, $attemptnumber, $readonly = false) {
        global $CFG;
        ...
        if ($readonly) {
            $grade = $seplment->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($seplment->get_context()->id, 'seplfeedback_editpdf',
        ...
        $file = self::get_combined_pdf_for_attempt($seplment, $userid, $attemptnumber);
        if (!$file) {
            \print_error('Could not generate combined pdf.');
        }

        // Store the combined pdf file somewhere to be opened by tcpdf.
        $tmpdir = \make_temp_directory('seplfeedback_editpdf/pagetotal/'
            . self::hash($seplment, $userid, $attemptnumber));
     ...
     * @return array(stored_file)
     */
    public static function generate_page_images_for_attempt($seplment, $userid, $attemptnumber) {
        global $CFG;
        ...
        $tmpdir = \make_temp_directory('seplfeedback_editpdf/pageimages/' . self::hash($seplment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
     ...
     * @return array(stored_file)
     */
    public static function get_page_images_for_attempt($seplment, $userid, $attemptnumber, $readonly = false) {
        ...
        if ($readonly) {
            ...
            if ($fs->is_area_empty($contextid, $component, $filearea, $itemid)) {
                ...
                self::generate_page_images_for_attempt($seplment, $userid, $attemptnumber);
                self::copy_pages_to_readonly_area($seplment, $grade);
            }
        }
        ...
        if (empty($pages)) {
            ...
            $pages = self::generate_page_images_for_attempt($seplment, $userid, $attemptnumber);
        }
     ...
     * @return string
     */
    protected static function get_downloadable_feedback_filename($seplment, $userid, $attemptnumber) {
        global $DB;

        $seplment = self::get_seplment_from_param($seplment);

        $groupmode = groups_get_activity_groupmode($seplment->get_course_module());
        $groupname = '';
        if ($groupmode) {
            $groupid = groups_get_activity_group($seplment->get_course_module(), true);
        ...
        if ($seplment->is_blind_marking()) {
            ...
            $prefix = clean_filename($prefix . '_' . $seplment->get_uniqueid_for_user($userid) . '_');
        } else {
     ...
     * @return stored_file
     */
    public static function generate_feedback_document($seplment, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('seplfeedback_editpdf/final/' . self::hash($seplment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
        ...
        $fs = \get_file_storage();
        $stamptmpdir = \make_temp_directory('seplfeedback_editpdf/stamps/' . self::hash($seplment, $userid, $attemptnumber));
        $grade = $seplment->get_user_grade($userid, true, $attemptnumber);
        // Copy any new stamps to this instance.
        if ($files = $fs->get_area_files($seplment->get_context()->id,
        ...
        $filename = self::get_downloadable_feedback_filename($seplment, $userid, $attemptnumber);
     ...
     *
     * @param int|\sepl $seplment The seplment.
     * @param \stdClass $grade The grade record.
     * @return void
     */
    public static function copy_pages_to_readonly_area($seplment, $grade) {
        $fs = get_file_storage();
        $seplment = self::get_seplment_from_param($seplment);
        $contextid = $seplment->get_context()->id;
        $component = 'seplfeedback_editpdf';
     ...
     * @return stored_file
     */
    public static function get_feedback_document($seplment, $userid, $attemptnumber) {
     ...
     * @return bool
     */
    public static function delete_feedback_document($seplment, $userid, $attemptnumber) {
