 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class seplfeedback_editpdf_renderer extends plugin_renderer_base {
    ...
    private function render_toolbar_button($icon, $tool, $accesskey = null, $disabled=false) {
        ...
        $iconalt = get_string('toolbarbutton', 'seplfeedback_editpdf', $alttext);

        $iconhtml = $this->pix_icon($icon, $iconalt, 'seplfeedback_editpdf');
     ...
     *
     * @param seplfeedback_editpdf_widget $widget - Renderable widget containing seplment, user and attempt number.
     * @return string
     */
    public function render_seplfeedback_editpdf_widget(seplfeedback_editpdf_widget $widget) {
        global $CFG;

        $html = '';

        $html .= html_writer::div(get_string('jsrequired', 'seplfeedback_editpdf'), 'hiddenifjs');
        $linkid = html_writer::random_id();
        if ($widget->readonly) {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('viewfeedbackonline', 'seplfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        } else {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('launcheditor', 'seplfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn', 'href'=>'#'));
        }
        $links = $launcheditorlink;

        $links .= html_writer::tag('div',
                                   get_string('unsavedchanges', 'seplfeedback_editpdf'),
                                   array('class'=>'seplfeedback_editpdf_unsavedchanges warning'));

        $html .= html_writer::div($links, 'visibleifjs');
        $header = get_string('pluginname', 'seplfeedback_editpdf');
        $body = '';
        ...
        $iconalt = get_string('navigateprevious', 'seplfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_prev, $iconalt, 'seplfeedback_editpdf');
        ...
        $navigation1 .= html_writer::tag('select', null, array('disabled'=>'true',
            'aria-label' => get_string('gotopage', 'seplfeedback_editpdf'), 'class'=>'navigate-page-select',
            'accesskey' => $this->get_shortcut('navigate-page-select')));
        $iconalt = get_string('navigatenext', 'seplfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_next, $iconalt, 'seplfeedback_editpdf');
        $navigation1 .= html_writer::tag('button', $iconhtml, array('disabled'=>'true',
        ...
        $progressbar = html_writer::div($progressbar, 'progress progress-info progress-striped active',
            array('title' => get_string('loadingeditor', 'seplfeedback_editpdf'),
                  'role'=> 'progressbar', 'aria-valuenow' => 0, 'aria-valuemin' => 0,
                  'aria-valuemax' => 100));
        $progressbarlabel = html_writer::div(get_string('generatingpdf', 'seplfeedback_editpdf'),
            'progressbarlabel');
        ...
        $this->page->requires->yui_module('moodle-seplfeedback_editpdf-editor',
                                          'M.seplfeedback_editpdf.editor.init',
                                          $editorparams);
