...
/**
 * This file contains the editor class for the seplfeedback_editpdf plugin
 *
...
class page_editor {
    ...
    public static function get_comments($gradeid, $pageno, $draft) {
        ...
        $records = $DB->get_records('seplfeedback_editpdf_cmnt', $params);
    ...
    public static function set_comments($gradeid, $pageno, $comments) {
        ...
        $DB->delete_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'pageno'=>$pageno, 'draft'=>1));
    ...
    public static function get_comment($commentid) {
        $record = $DB->get_record('seplfeedback_editpdf_cmnt', array('id'=>$commentid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function add_comment(comment $comment) {
        global $DB;
        $comment->id = null;
        return $DB->insert_record('seplfeedback_editpdf_cmnt', $comment);
    }
    ...
    public static function remove_comment($commentid) {
        global $DB;
        return $DB->delete_records('seplfeedback_editpdf_cmnt', array('id'=>$commentid));
    }
    ...
    public static function get_annotations($gradeid, $pageno, $draft) {
        ...
        $annotations = array();
        $records = $DB->get_records('seplfeedback_editpdf_annot', $params);
        foreach ($records as $record) {
    ...
    public static function set_annotations($gradeid, $pageno, $annotations) {
        ...
        $DB->delete_records('seplfeedback_editpdf_annot', array('gradeid' => $gradeid, 'pageno' => $pageno, 'draft' => 1));
        $added = 0;
    ...
    public static function get_annotation($annotationid) {
        ...
        $record = $DB->get_record('seplfeedback_editpdf_annot', array('id'=>$annotationid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function unrelease_drafts($gradeid) {
        ...
        $result = $DB->delete_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $result = $DB->delete_records('seplfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0)) && $result;
        return $result;
    }
    ...
    public static function release_drafts($gradeid) {
        ...
        $DB->delete_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $DB->delete_records('seplfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('seplfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('seplfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('seplfeedback_editpdf_cmnt', $record);
        }
    ...
    public static function has_annotations_or_comments($gradeid, $includedraft) {
        ...
        if ($DB->count_records('seplfeedback_editpdf_cmnt', $params)) {
            return true;
        }
        if ($DB->count_records('seplfeedback_editpdf_annot', $params)) {
            return true;
        }
        return false;
    }
    ...
    public static function revert_drafts($gradeid) {
        ...
        // Delete the previous non-draft annotations and comments.
        $DB->delete_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        $DB->delete_records('seplfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('seplfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('seplfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('seplfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('seplfeedback_editpdf_annot', $record);
    ...
    public static function add_annotation(annotation $annotation) {
        ...
        $annotation->id = null;
        return $DB->insert_record('seplfeedback_editpdf_annot', $annotation);
    }
    ...
    public static function remove_annotation($annotationid) {
        ...
        return $DB->delete_records('seplfeedback_editpdf_annot', array('id'=>$annotationid));
    }
     ...
     * @param int $contextid The ID of the context for the seplment.
    ...
    public static function delete_draft_content($gradeid) {
        ...
        $result = $DB->delete_records('seplfeedback_editpdf_annot', $conditions);
        $result = $result && $DB->delete_records('seplfeedback_editpdf_cmnt', $conditions);
        return $result;
    }
}
