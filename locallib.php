...
/**
 * This file contains the definition for the class seplment
 *
...
require_once($CFG->dirroot . '/mod/sepl/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/sepl/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/sepl/submissionplugin.php');
...
/**
 * Standard base class for mod_sepl (seplment types).
 *
 * @package   mod_sepl
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class sepl {
    ...
    /** @var stdClass the seplment record that contains the global settings for this sepl instance */
    private $instance;

    /** @var stdClass the grade_item record for this sepl instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this sepl instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this sepl instance belongs to */
    private $course;

    /** @var stdClass the admin config for all sepl instances  */
    private $adminconfig;

    /** @var sepl_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this sepl instance */
    ...
    /** @var array cached list of participants for this seplment. The cache key will be group, showactive and the context id */
    ...
    /**
     * Constructor for the base sepl class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('seplsubmission');
        $this->feedbackplugins = $this->load_plugins('seplfeedback');
    }
    ...
    /**
     * Based on the current seplment settings should we display the intro.
     *
    ...
    /**
     * Does an seplment have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype seplsubmission | seplfeedback
     * @param string $type
     * @return mixed sepl_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('sepl'));
     ...
     * @return mixed sepl_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('seplfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed sepl_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('seplsubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('sepl'));
                $pluginclass = 'sepl_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof sepl_plugin) {
    ...
    /**
     * Display the seplment, used by view.php
     *
     * The seplment is displayed differently depending on your role,
     * the settings for the seplment and the status of the seplment.
     *
    ...
    public function view($action='') {
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/sepl/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'sepl');
        ...
        } else if ($action == 'viewpluginseplfeedback') {
            $o .= $this->view_plugin_content('seplfeedback');
        } else if ($action == 'viewpluginseplsubmission') {
            $o .= $this->view_plugin_content('seplsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'sepl'), $notices);
        } else {
     ...
     *             when upgrading an old seplment to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $returnid = $DB->insert_record('sepl', $update);
        $this->instance = $DB->get_record('sepl', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        $DB->update_record('sepl', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this seplment.
     *
     * @return bool
     */
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this seplment.
        $fs = get_file_storage();
        ...
        $DB->delete_records('sepl_submission', array('seplment' => $this->get_instance()->id));
        $DB->delete_records('sepl_grades', array('seplment' => $this->get_instance()->id));
        $DB->delete_records('sepl_plugin_config', array('seplment' => $this->get_instance()->id));
        $DB->delete_records('sepl_user_flags', array('seplment' => $this->get_instance()->id));
        $DB->delete_records('sepl_user_mapping', array('seplment' => $this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('sepl', array('id'=>$this->get_instance()->id));
     ...
     * seplment submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'sepl');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_sepl_submissions)) {
            // Delete files associated with this seplment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'sepl'),
            ...
            $seplids = $DB->get_records('sepl', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($seplids));

            $DB->delete_records_select('sepl_submission', "seplment $sql", $params);
            $DB->delete_records_select('sepl_user_flags', "seplment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('sepl_grades', "seplment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/sepl/lib.php');
                sepl_reset_gradebook($data->courseid);
                ...
                if ($this->get_instance()->blindmarking && $this->get_instance()->revealidentities) {
                    $DB->set_field('sepl', 'revealidentities', 0, array('id' => $this->get_instance()->id));
                }
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('sepl',
     ...
     *
     * @param sepl_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(sepl_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this seplment.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this seplment
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        $sepl->cmidnumber = $coursemoduleid;
        ...
        return sepl_grade_item_update($sepl, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('sepl');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this seplment.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'sepl', 'instance'=>$instance->id);
            ...
            $DB->delete_records('event', array('modulename'=>'sepl', 'instance'=>$instance->id));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('sepl', $update);
        $this->instance = $DB->get_record('sepl', array('id'=>$update->id), '*', MUST_EXIST);

        $this->save_intro_draft_files($formdata);

        // Load the seplment so the plugins have access to it.
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param sepl_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(sepl_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'sepl'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'sepl'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'sepl'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'sepl'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    /**
     * Has this seplment been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this seplment
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('sepl', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the seplment class. ' .
                                       'Cannot load the seplment record.');
        }
        return $this->instance;
    }

    /**
     * Get the primary grade item for this sepl instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    public function list_participants($currentgroup, $idsonly) {
        ...
        if (!isset($this->participants[$key])) {
            $users = get_enrolled_users($this->context, 'mod/sepl:submit', $currentgroup, 'u.*', null, null, null,
    ...
    /**
     * Load a count of valid teams for this seplment.
     *
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group seplment because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/sepl:submit', $currentgroup, true);

        $params['seplid'] = $this->get_instance()->id;
    ...
    public function count_submissions($includenew = false) {
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/sepl:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status) {
        ...
        $params['seplid'] = $this->get_instance()->id;
        $params['seplid2'] = $this->get_instance()->id;
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'seplment_');
        // Zip files.
    ...
    public static function cron() {
        ...
        if (!empty($submissions)) {
            ...
            foreach ($submissions as $submission) {
                ...
                self::send_seplment_notification($grader,
                                                   $user,
                                                   $messagetype,
                                                   $eventtype,
                                                   $updatetime,
    ...
    /**
     * Update user flags for this user in this seplment.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->seplment <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('sepl_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the seplment and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @param bool $reopenattempt If the attempt reopen method is manual, allow another attempt at this seplment.
     * @return bool true for success
     */
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('sepl_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_sepl\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/sepl/extensionform.php');
        ...
        $header = new sepl_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'sepl'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_sepl_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new sepl_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the seplment.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'sepl') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('seplment'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        $submission = null;
        $submissions = $DB->get_records('sepl_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            $submission->seplment = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('sepl_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('sepl_submission', 'latest', 0, $params);
            }
            $submission->status = ASSIGN_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('sepl_submission', $submission);
            return $DB->get_record('sepl_submission', array('id' => $sid));
        }
        return false;
    }

    /**
     * View a summary listing of all seplments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('sepl', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new sepl_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['sepl'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/sepl:grade', $context)) {
                $submitted = $seplment->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/sepl:submit', $context)) {
                $usersubmission = $seplment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'sepl');
                } else {
                    $submitted = get_string('submissionstatus_', 'sepl');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'sepl', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_sepl_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team seplments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'seplsubmission') {
            ...
            $o .= $this->get_renderer()->render(new sepl_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new sepl_submission_plugin_submission($plugin,
                                                              $item,
                                                              sepl_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_sepl\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new sepl_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new sepl_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              sepl_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_sepl\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param sepl_plugin $plugin - The seplment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'sepl');
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/sepl:exportownsubmission', $this->context)) {
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new sepl_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'sepl')));
        $gradingresult = new sepl_gradingmessage(get_string('savegradingresult', 'sepl'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new sepl_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'sepl')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new sepl_gradingmessage(get_string('quickgradingresult', 'sepl'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_submission($userid) {
        if (!$this->can_view_submission($userid)) {
            throw new required_capability_exception($this->context, 'mod/sepl:viewgrades', 'nopermission', '');
        }
    }

    /**
     * Throw an error if the permissions to view grades in this seplment are missing.
     *
     * @throws required_capability_exception
     * @return none
     */
    ...
    /**
     * Does this user have view grade or grade permission for this seplment?
     *
     * @return bool
     */
    public function can_view_grades() {
        // Permissions check.
        if (!has_any_capability(array('mod/sepl:viewgrades', 'mod/sepl:grade'), $this->context)) {
            return false;
        }
    ...
    /**
     * Does this user have grade permission for this seplment?
     *
     * @return bool
     */
    public function can_grade() {
        // Permissions check.
        if (!has_capability('mod/sepl:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all seplment submissions.
     *
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/sepl:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'sepl') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'sepl'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new sepl_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'sepl'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'sepl'));
            $url = new moodle_url('/mod/sepl/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_sepl\event\all_submissions_downloaded::create_from_sepl($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_sepl add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
     ...
     *
     * For team seplments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('seplment'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('seplment'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('sepl_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('seplment'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('sepl_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->seplment = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('sepl_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('sepl_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->seplment   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('sepl_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('seplment'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('sepl_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        require_once($CFG->dirroot . '/mod/sepl/gradeform.php');

        // Need submit permission to submit an seplment.
        require_capability('mod/sepl:grade', $this->context);

        $header = new sepl_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'sepl'));
        $o .= $this->get_renderer()->render($header);
        ...
        $cache = cache::make_from_params(cache_store::MODE_SESSION, 'mod_sepl', 'useridlist');
        if (!$userid) {
        ...
        if ($user) {
            ...
            $usersummary = new sepl_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new sepl_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             sepl_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             false,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'sepl', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_sepl_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new sepl_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new sepl_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
        ...
        \mod_sepl\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/sepl:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/sepl/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/sepl/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'sepl'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_sepl\event\reveal_identities_confirmation_page_viewed::create_from_sepl($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/sepl/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this seplment.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/sepl/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/sepl/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/sepl/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'sepl');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/sepl/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'sepl');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/sepl:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/sepl/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'sepl');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/sepl/view.php' .
                           '?id=' .  $cmid .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'sepl'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_sepl', 'submissions');

        $perpage = (int) get_user_preferences('sepl_perpage', 10);
        $filter = get_user_preferences('sepl_filter', '');
        $markerfilter = get_user_preferences('sepl_markerfilter', '');
        $workflowfilter = get_user_preferences('sepl_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('sepl_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/sepl:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/sepl:grade', '', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'sepl');
            $markingallocationoptions[ASSIGN_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'sepl');
        ...
        if ($markingworkflow) {
            ...
            $markingworkflowoptions[''] = get_string('filternone', 'sepl');
        ...
        $gradingoptionsform = new mod_sepl_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_sepl_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new sepl_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_sepl_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new sepl_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new sepl_grading_table($this, $perpage, $filter, 0, false);
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $seplform = new sepl_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($seplform);
        }
        $seplform = new sepl_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_sepl.init_grading_options');
        $o .= $this->get_renderer()->render($seplform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        \mod_sepl\event\grading_table_viewed::create_from_sepl($this)->trigger();
    ...
    /**
     * Message for students when seplment submissions have been closed.
     *
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/sepl/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/sepl:viewblinddetails', $this->get_context());
            if ($hasviewblind) {
                return fullname($user);
            } else {
                $uniqueid = $this->get_uniqueid_for_user($user->id);
                return get_string('participant', 'sepl') . ' ' . $uniqueid;
            }
        } else {
            return fullname($user);
        }
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/sepl/submission_form.php');
        // Need submit permission to submit an seplment.
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/sepl:submit', $this->context);
            $title = get_string('editsubmission', 'sepl');
        } else {
            ...
            $title = get_string('editsubmissionother', 'sepl', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'sepl'));
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_sepl_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new sepl_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_sepl\event\submission_form_viewed::create_from_user($this, $user)->trigger();

        return $o;
    }

    /**
     * See if this seplment has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/sepl:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_sepl_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        foreach ($userlist as $userid) {
            if ($usercount >= 5) {
                $usershtml .= get_string('moreusers', 'sepl', count($userlist) - 5);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usershtml .= $this->get_renderer()->render(new sepl_user_summary($user,
                                                                $this->get_course()->id,
        ...
        $header = new sepl_header($this->get_instance(),
            ...
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'sepl'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new sepl_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_sepl\event\batch_set_workflow_state_viewed::create_from_sepl($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        list($sort, $params) = users_order_by_sql();
        $markers = get_users_by_capability($this->get_context(), 'mod/sepl:grade', '', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_sepl_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new sepl_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'sepl'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new sepl_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_sepl\event\batch_set_marker_allocation_viewed::create_from_sepl($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/sepl/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_sepl_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new sepl_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_sepl\event\submission_confirmation_form_viewed::create_from_sepl($this)->trigger();
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_sepl', 'submissions');
            ...
            $submissionstatus = new sepl_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              sepl_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
                                                              ...
                                                              $instance->preventsubmissionnotingroup,
                                                              $usergroups);
            if (has_capability('mod/sepl:submit', $this->get_context(), $user)) {
                $o .= $this->get_renderer()->render($submissionstatus);
            }
            ...
            $cangrade = has_capability('mod/sepl:grade', $this->get_context());
            ...
            if ((!is_null($gradebookgrade->grade) || !$emptyplugins)
                ...
                $feedbackstatus = new sepl_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // The seplment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('sepl_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('sepl_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new sepl_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new sepl_plugin_header($plugin));
            }
        }
        ...
        if ($this->can_view_grades()) {
            ...
            $currenturl = new moodle_url('/mod/sepl/view.php', array('id' => $this->get_course_module()->id));
            ...
            if ($instance->teamsubmission) {
                ...
                $summary = new sepl_grading_summary($this->count_teams($activitygroup),
                ...
                $summary = new sepl_grading_summary($countparticipants,
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        \mod_sepl\event\submission_status_viewed::create_from_sepl($this)->trigger();
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this seplment qualifies as submitted or draft.
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_REOPENED) {
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('sepl_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $result = $DB->update_record('sepl_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('sepl_submission', $submission) && $result;
        }
    ...
    /**
     * Is this seplment open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the seplment locked?
     *
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the seplment.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->sepl_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid &&
                ...
                has_capability('mod/sepl:submit', $this->context, $graderid)) {
            // User can edit their own submission.
            return true;
        }

        if (!has_capability('mod/sepl:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/sepl:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/sepl:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($seplmentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'sepl', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/sepl/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/sepl/view.php?id=' . $coursemodule->id . '">' .
                     format_string($seplmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'sepl', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_seplment_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'sepl') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'sepl');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->seplment = format_string($seplmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/sepl/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'sepl', $info);
        ...
        $eventdata->component       = 'mod_sepl';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->seplment;
    ...
    public function send_notification($userfrom,
                                      ...
                                      $eventtype,
                                      $updatetime) {
        self::send_seplment_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'sepl');
            return false;
        }
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_sepl\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_sepl\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an seplment.
        require_capability('mod/sepl:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/sepl:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_sepl\event\extension_granted::create_from_sepl($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_extension(& $mform) {
        ...
        $mform = new mod_sepl_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'sepl');
        }
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified, uf.workflowstate, uf.allocatedmarker
                ...
                LEFT JOIN {sepl_grades} g ON
                    u.id = g.userid AND
                    g.seplment = :seplid2 AND
                    g.attemptnumber = gmx.maxattempt
                LEFT JOIN {sepl_user_flags} uf ON uf.seplment = g.seplment AND uf.userid = g.userid
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'sepl');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('seplfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_sepl\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/sepl',
                                          $this->course->id,
                                          'mod',
                                          'sepl',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'sepl');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/sepl:revealidentities', $this->context);

        if ($this->get_instance()->revealidentities || empty($this->get_instance()->blindmarking)) {
            return false;
        }

        // Update the seplment record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('seplfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('sepl_grades', array('seplment'=>$this->get_instance()->id));
        ...
        \mod_sepl\event\identities_revealed::create_from_sepl($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_sepl_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('sepl_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('sepl_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('sepl_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('sepl_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('sepl_quickgrading', isset($formdata->quickgrading));
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'sepl', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'sepl');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'sepl');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'sepl');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'sepl', $params) . ' <br>';
    ...
    /**
     * Copy the current seplment submission from the last submitted attempt.
     *
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     * @return bool
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != ASSIGN_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'sepl');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'sepl');
            return false;
        }
        ...
        \mod_sepl\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted seplments
            // and they haven't been notified.
            \mod_sepl\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    /**
     * Save seplment submission for the current user.
     *
     * @param  stdClass $data
     * @param  array $notices Any error messages that should be shown
     *                        to the user.
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'sepl');
            return true;
        }
        ...
        if ($pluginerror || $allempty) {
            if ($allempty) {
                $notices[] = get_string('submissionempty', 'mod_sepl');
            }
            return false;
        }
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        require_sesskey();
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('duedatereached', 'sepl');
            return false;
        }
        $instance = $this->get_instance();
     ...
     * This is specific to the seplment, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'sepl', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'sepl');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $mform->addElement('hidden', 'grade', $name);
                    $mform->hardFreeze('grade');
                    $mform->setType('grade', PARAM_RAW);
                    $strgradelocked = get_string('gradelocked', 'sepl');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'sepl');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'sepl')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'sepl'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'sepl');
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            list($sort, $params) = users_order_by_sql();
            $markers = get_users_by_capability($this->context, 'mod/sepl:grade', '', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'sepl'));
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'sepl'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'sepl');
        ...
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'sepl'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'sepl', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'sepl'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'sepl'));
        ...
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != ASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'sepl'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'sepl');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'sepl'), $attemptreopenmethod);
            ...
            if ($maxattempts == ASSIGN_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'sepl');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'sepl'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'sepl'), $attemptnumber + 1);
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'sepl'));
                $mform->setDefault('addattempt', 0);
            }
        }
        $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'sepl'));
        // Get seplment visibility information for student.
        $modinfo = get_fast_modinfo($settings->course, $userid);
        $cm = $modinfo->get_cm($this->get_course_module()->id);

        // Don't allow notification to be sent if the student can't access the seplment,
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'sepl');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'sepl');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'sepl');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'sepl');
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_sepl\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_sepl\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_sepl\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_sepl\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_sepl\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
     ...
     * @return bool
     */
    public static function can_upgrade_seplment($type, $version) {
        $seplment = new sepl(null, null, null);
        return $seplment->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an seplment from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old seplment files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old seplment context id
     * @param int $oldcomponent The old seplment component ('seplment')
     * @param int $oldfilearea The old seplment filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new seplment context id
     * @param int $newcomponent The new seplment component ('seplment')
     * @param int $newfilearea The new seplment filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group seplment
     * at most 1 time.
     *
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $seplmentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('seplfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('seplid1'=>$seplmentid,
                        'seplid2'=>$seplmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN {sepl_submission} s
                                                    ON u.id = s.userid and s.seplment = :seplid1 AND
                                                    s.latest = 1
                                                JOIN {sepl_grades} g
                                                    ON u.id = g.userid and g.seplment = :seplid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - sepl them a random id.
     *
     * @param int $seplid The seplid to lookup
     */
    public static function allocate_unique_ids($seplid) {
        global $DB;

        $cm = get_coursemodule_from_instance('sepl', $seplid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/sepl:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('sepl_user_mapping',
                                      array('seplment'=>$seplid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->seplment = $seplid;
                $record->userid = $user->id;
                $DB->insert_record('sepl_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this seplment.
     *
     * @param int $seplid The seplment id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($seplid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('seplment'=>$seplid, 'userid'=>$userid);
        if ($record = $DB->get_record('sepl_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($seplid);
        ...
        return $DB->insert_record('sepl_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this seplment.
     *
     * @param int $seplid The id of the seplment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($seplid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('sepl_user_mapping',
                                      array('seplment'=>$seplid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/sepl:grade', $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'sepl');
            $states[ASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'sepl');
        }
        if (has_any_capability(array('mod/sepl:reviewgrades',
                                     'mod/sepl:managegrades'), $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'sepl');
            $states[ASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'sepl');
        }
        if (has_any_capability(array('mod/sepl:releasegrades',
                                     'mod/sepl:managegrades'), $this->context)) {
            $states[ASSIGN_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'sepl');
        }
...
/**
 * Portfolio caller class for mod_sepl.
 *
 * @package   mod_sepl
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the seplment we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('sepl', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the seplment type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_sepl');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'seplment.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'sepl' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/sepl:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'sepl');
    }
