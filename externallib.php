 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sepl_external extends external_api {

    /**
     * Generate a warning in a standard structure for a known failure.
     *
     * @param int $seplmentid - The seplment
     ...
     */
    private static function generate_warning($seplmentid, $warningcode, $detail) {
        $warningmessages = array(
            'couldnotlock'=>'Could not lock the submission for this user.',
            'couldnotunlock'=>'Could not unlock the submission for this user.',
            'couldnotsubmitforgrading'=>'Could not submit seplment for grading.',
    ...
    public static function get_grades_parameters() {
        return new external_function_parameters(
            array(
                'seplmentids' => new external_multiple_structure(
                    new external_value(PARAM_INT, 'seplment id'),
                    '1 or more seplment ids',
                    VALUE_REQUIRED),
    ...
    /**
     * Returns grade information from sepl_grades for the requested seplment ids
     * @param int[] $seplmentids
     * @param int $since only return records with timemodified >= since
     * @return array of grade records for each requested seplment
     * @since  Moodle 2.4
     */
    public static function get_grades($seplmentids, $since = 0) {
        global $DB;
        $params = self::validate_parameters(self::get_grades_parameters(),
                        array('seplmentids' => $seplmentids,
                              'since' => $since));

        $seplments = array();
        $warnings = array();
        $requestedseplmentids = $params['seplmentids'];

        // Check the user is allowed to get the grades for the seplments requested.
        $placeholders = array();
        list($sqlseplmentids, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlseplmentids;
        ...
        foreach ($cms as $cm) {
            try {
                ...
                $requestedseplmentids = array_diff($requestedseplmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'seplment';
        ...
        if (count ($requestedseplmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);
            ...
            foreach ($rs as $rd) {
                ...
                if (is_null($currentseplmentid) || ($rd->seplment != $currentseplmentid )) {
                    if (!is_null($seplment)) {
                        $seplments[] = $seplment;
                    }
                    $seplment = array();
                    $seplment['seplmentid'] = $rd->seplment;
                    $seplment['grades'] = array();
                    $requestedseplmentids = array_diff($requestedseplmentids, array($rd->seplment));
                }
                $seplment['grades'][] = $grade;

                $currentseplmentid = $rd->seplment;
            }
            if (!is_null($seplment)) {
                $seplments[] = $seplment;
            }
        ...
        foreach ($requestedseplmentids as $seplmentid) {
            $warning = array();
            $warning['item'] = 'seplment';
            $warning['itemid'] = $seplmentid;
        ...
        $result = array();
        $result['seplments'] = $seplments;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Creates an sepl_grades external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.4
     */
    private static function sepl_grades() {
        return new external_single_structure(
            array (
                'seplmentid'    => new external_value(PARAM_INT, 'seplment id'),
    ...
    public static function get_grades_returns() {
        return new external_single_structure(
            array(
                'seplments' => new external_multiple_structure(self::sepl_grades(), 'list of seplment grade information'),
                'warnings'      => new external_warnings('item is always \'seplment\'',
                    'when errorcode is 3 then itemid is an seplment id. When errorcode is 1, itemid is a course module id',
     ...
     * @since  Moodle 2.4
     */
    public static function get_seplments_parameters() {
        return new external_function_parameters(
            array(
    ...
    public static function get_seplments($courseids = array(), $capabilities = array()) {
        ...
        $extrafields='m.id as seplmentid, ' .
                     'm.course, ' .
                     'm.nosubmissions, ' .
                     'm.submissiondrafts, ' .
                     'm.sendnotifications, '.
            ...
            if ($modules = get_coursemodules_in_course('sepl', $courses[$id]->id, $extrafields)) {
                foreach ($modules as $module) {
                    ...
                    foreach ($configrecords as $configrecord) {
                        $configarray[] = array(
                            'id' => $configrecord->id,
                            'seplment' => $configrecord->seplment,
                    ...
                    if ($sepl->show_intro()) {
                        ...
                        list($seplment['intro'], $seplment['introformat']) = external_format_text($module->intro,
                            $module->introformat, $context->id, 'mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA, 0);

                        $fs = get_file_storage();
                        if ($files = $fs->get_area_files($context->id, 'mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA,
                            ...
                            foreach ($files as $file) {
                                ...
                                $seplment['introattachments'][] = array(
                                    ...
                                    'fileurl'  => moodle_url::make_webservice_pluginfile_url(
                                        $context->id, 'mod_sepl', ASSIGN_INTROATTACHMENT_FILEAREA, 0, '/', $filename)->out(false)
                                );
                            }
                        }
                    }
    ...
    /**
     * Creates an seplment external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_seplments_seplment_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'seplment id'),
                'cmid' => new external_value(PARAM_INT, 'course module id'),
                'course' => new external_value(PARAM_INT, 'course id'),
                'name' => new external_value(PARAM_TEXT, 'seplment name'),
                ...
                'duedate' => new external_value(PARAM_INT, 'seplment due date'),
                ...
                'timemodified' => new external_value(PARAM_INT, 'last time seplment was modified'),
                ...
                'revealidentities' => new external_value(PARAM_INT, 'show identities for a blind marking seplment'),
                ...
                'configs' => new external_multiple_structure(self::get_seplments_config_structure(), 'configuration settings'),
                'intro' => new external_value(PARAM_RAW,
                    'seplment intro, not allways returned because it deppends on the activity configuration', VALUE_OPTIONAL),
            ...
            ), 'seplment information object');
    }

    /**
     * Creates an sepl_plugin_config external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_seplments_config_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'sepl_plugin_config id'),
                'seplment' => new external_value(PARAM_INT, 'seplment id'),
                'plugin' => new external_value(PARAM_TEXT, 'plugin'),
                'subtype' => new external_value(PARAM_TEXT, 'subtype'),
                'name' => new external_value(PARAM_TEXT, 'name'),
                'value' => new external_value(PARAM_TEXT, 'value')
            ), 'seplment configuration object'
        );
    }
     ...
     * @since Moodle 2.4
     */
    private static function get_seplments_course_structure() {
        return new external_single_structure(
            array(
                ...
                'seplments' => new external_multiple_structure(self::get_seplments_seplment_structure(), 'seplment info')
              ), 'course information object'
        );
    }

    /**
     * Describes the return value for get_seplments
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    public static function get_seplments_returns() {
        return new external_single_structure(
            array(
                'courses' => new external_multiple_structure(self::get_seplments_course_structure(), 'list of courses'),
    ...
    public static function get_submissions($seplmentids, $status = '', $since = 0, $before = 0) {
        ...
        foreach ($sepls as $sepl) {
            ...
            $placeholders = array('seplid1' => $sepl->get_instance()->id,
                                  'seplid2' => $sepl->get_instance()->id);
            ...
            $sql = "SELECT mas.id, mas.seplment,mas.userid,".
                   "mas.timecreated,mas.timemodified,mas.status,mas.groupid,mas.attemptnumber ".
                   "FROM {sepl_submission} mas ".
                   "JOIN ( " . $submissionmaxattempt . " ) smx ON mas.userid = smx.userid ".
                   "WHERE mas.seplment = :seplid2 AND mas.attemptnumber = smx.maxattempt";
                    ...
                    foreach ($submissionplugins as $submissionplugin) {
                        ...
                        foreach ($fileareas as $filearea => $name) {
                            ...
                            foreach ($files as $file) {
                                ...
                                $fileurl = file_encode_url($CFG->wwwroot . '/webservice/pluginfile.php', '/' . $sepl->get_context()->id .
            ...
            $seplments[] = array(
                'seplmentid' => $sepl->get_instance()->id,
    ...
    /**
     * Creates an sepl_submissions external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    ...
    public static function get_submissions_returns() {
        return new external_single_structure(
            array(
                'seplments' => new external_multiple_structure(self::get_submissions_structure(), 'seplment submissions'),
    ...
    /**
     * Returns user flag information from sepl_user_flags for the requested seplment ids
     * @param int[] $seplmentids
     * @return array of user flag records for each requested seplment
     * @since  Moodle 2.6
     */
    public static function get_user_flags($seplmentids) {
        global $DB;
        ...
        // Check the user is allowed to get the user flags for the seplments requested.
        $placeholders = array();
        list($sqlseplmentids, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlseplmentids;
        ...
        // Create the query and populate an array of sepl_user_flags records from the recordset results.
        if (count ($requestedseplmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT auf.id,auf.seplment,auf.userid,auf.locked,auf.mailed,".
                   "auf.extensionduedate,auf.workflowstate,auf.allocatedmarker ".
                   "FROM {sepl_user_flags} auf ".
                   "WHERE auf.seplment ".$inorequalsql.
                   " ORDER BY auf.seplment, auf.id";
            ...
            foreach ($rs as $rd) {
                ...
                if (is_null($currentseplmentid) || ($rd->seplment != $currentseplmentid )) {
                    ...
                    $seplment = array();
                    $seplment['seplmentid'] = $rd->seplment;
                    $seplment['userflags'] = array();
                    $requestedseplmentids = array_diff($requestedseplmentids, array($rd->seplment));
                }
                $seplment['userflags'][] = $userflag;
    ...
    /**
     * Creates an sepl_user_flags external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function sepl_user_flags() {
        return new external_single_structure(
            array (
                'seplmentid'    => new external_value(PARAM_INT, 'seplment id'),
    ...
    public static function get_user_flags_returns() {
        return new external_single_structure(
            array(
                'seplments' => new external_multiple_structure(self::sepl_user_flags(), 'list of sepl user flag information'),
                'warnings'      => new external_warnings('item is always \'seplment\'',
                    'when errorcode is 3 then itemid is an seplment id. When errorcode is 1, itemid is a course module id',
    ...
    /**
     * Returns user mapping information from sepl_user_mapping for the requested seplment ids
     * @param int[] $seplmentids
     * @return array of user mapping records for each requested seplment
     * @since  Moodle 2.6
     */
    public static function get_user_mappings($seplmentids) {
        global $DB;
        ...
        // Check the user is allowed to get the mappings for the seplments requested.
        $placeholders = array();
        list($sqlseplmentids, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);
        $sql = "SELECT cm.id, cm.instance FROM {course_modules} cm JOIN {modules} md ON md.id = cm.module ".
               "WHERE md.name = :modname AND cm.instance ".$sqlseplmentids;
        ...
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/sepl:revealidentities', $context);
            } catch (Exception $e) {
                $requestedseplmentids = array_diff($requestedseplmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'seplment';
        ...
        // Create the query and populate an array of sepl_user_mapping records from the recordset results.
        if (count ($requestedseplmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedseplmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT aum.id,aum.seplment,aum.userid ".
                   "FROM {sepl_user_mapping} aum ".
                   "WHERE aum.seplment ".$inorequalsql.
                   " ORDER BY aum.seplment, aum.id";
            ...
            foreach ($rs as $rd) {
                ...
                if (is_null($currentseplmentid) || ($rd->seplment != $currentseplmentid )) {
                    ...
                    $seplment = array();
                    $seplment['seplmentid'] = $rd->seplment;
                    $seplment['mappings'] = array();
                    $requestedseplmentids = array_diff($requestedseplmentids, array($rd->seplment));
                }
                $seplment['mappings'][] = $mapping;
    ...
    /**
     * Creates an sepl_user_mappings external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function sepl_user_mappings() {
        return new external_single_structure(
            array (
                'seplmentid'    => new external_value(PARAM_INT, 'seplment id'),
    ...
    public static function get_user_mappings_returns() {
        return new external_single_structure(
            array(
                'seplments' => new external_multiple_structure(self::sepl_user_mappings(), 'list of sepl user mapping data'),
                'warnings'      => new external_warnings('item is always \'seplment\'',
                    'when errorcode is 3 then itemid is an seplment id. When errorcode is 1, itemid is a course module id',
    ...
    public static function lock_submissions_parameters() {
        return new external_function_parameters(
            array(
                'seplmentid' => new external_value(PARAM_INT, 'The seplment id to operate on'),
    ...
    /**
     * Locks (prevent updates to) submissions in this seplment.
     *
     ...
     * @since Moodle 2.6
     */
    public static function lock_submissions($seplmentid, $userids) {
        global $CFG;
        require_once("$CFG->dirroot/mod/sepl/locallib.php");
        ...
        foreach ($params['userids'] as $userid) {
            if (!$seplment->lock_submission($userid)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['seplmentid'];
                $warnings[] = self::generate_warning($params['seplmentid'],
                                                     'couldnotlock',
                                                     $detail);
            }
        }
     ...
     * @since Moodle 2.6
     */
    public static function unlock_submissions($seplmentid, $userids) {
        global $CFG;
        require_once("$CFG->dirroot/mod/sepl/locallib.php");
        ...
        $warnings = array();
        foreach ($params['userids'] as $userid) {
            if (!$seplment->unlock_submission($userid)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['seplmentid'];
                $warnings[] = self::generate_warning($params['seplmentid'],
                                                     'couldnotunlock',
                                                     $detail);
    ...
    public static function submit_for_grading_parameters() {
        return new external_function_parameters(
            array(
                'seplmentid' => new external_value(PARAM_INT, 'The seplment id to operate on'),
                'acceptsubmissionstatement' => new external_value(PARAM_BOOL, 'Accept the seplment submission statement')
            )
        );
    }

    /**
     * Submit the logged in users seplment for grading.
     *
     * @param int $seplmentid The id of the seplment
     * @return array of warnings to indicate any errors.
     * @since Moodle 2.6
     */
    public static function submit_for_grading($seplmentid, $acceptsubmissionstatement) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/sepl/locallib.php");
        ...
        if (!$seplment->submit_for_grading($data, $notices)) {
            $detail = 'User id: ' . $USER->id . ', Assignment id: ' . $params['seplmentid'] . ' Notices:' . implode(', ', $notices);
            $warnings[] = self::generate_warning($params['seplmentid'],
    ...
    /**
     * Grant extension dates to students for an seplment.
     *
     * @param int $seplmentid The id of the seplment
     ...
     * @since Moodle 2.6
     */
    public static function save_user_extensions($seplmentid, $userids, $dates) {
        global $CFG;
        require_once("$CFG->dirroot/mod/sepl/locallib.php");
        ...
        foreach ($params['userids'] as $idx => $userid) {
            ...
            if (!$seplment->save_user_extension($userid, $duedate)) {
                $detail = 'User id: ' . $userid . ', Assignment id: ' . $params['seplmentid'] . ', Extension date: ' . $duedate;
                $warnings[] = self::generate_warning($params['seplmentid'],
        ...
        return new external_function_parameters(
            array(
                ...
                'applytoall' => new external_value(PARAM_BOOL, 'If true, this grade will be applied ' .
                                                               'to all members ' .
                                                               'of the group (for group seplments).'),
    ...
    /**
     * Save a student grade for a single seplment.
     *
     * @param int $seplmentid The id of the seplment
     * @param int $userid The id of the user
     * @param float $grade The grade (ignored if the seplment uses advanced grading)
     ...
     * @since Moodle 2.6
     */
    public static function save_grade($seplmentid,
                                      $userid,
                                      $grade,
                                      $attemptnumber,
                                      $addattempt,
        ...
        $seplment->save_grade($params['userid'], $gradedata);
     ...
     * @since Moodle 2.6
     */
    public static function copy_previous_attempt($seplmentid) {
        global $CFG, $USER;
        require_once("$CFG->dirroot/mod/sepl/locallib.php");
        ...
        $seplment->copy_previous_attempt($submissiondata, $notices);

        $warnings = array();
        foreach ($notices as $notice) {
            $warnings[] = self::generate_warning($seplmentid,
    ...
    public static function view_grading_table_parameters() {
        return new external_function_parameters(
            array(
                'seplid' => new external_value(PARAM_INT, 'sepl instance id')
            )
        );
    }
