 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sepl_grading_options_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('gradingoptions', 'sepl'));
        // Visible elements.
        $options = array(-1=>get_string('all'), 10=>'10', 20=>'20', 50=>'50', 100=>'100');
        $mform->addElement('select', 'perpage', get_string('seplmentsperpage', 'sepl'), $options, $dirtyclass);
        $options = array('' => get_string('filternone', 'sepl'),
                         ASSIGN_FILTER_NOT_SUBMITTED => get_string('filternotsubmitted', 'sepl'),
                         ASSIGN_FILTER_SUBMITTED => get_string('filtersubmitted', 'sepl'),
                         ASSIGN_FILTER_REQUIRE_GRADING => get_string('filterrequiregrading', 'sepl'));
        if ($instance['submissionsenabled']) {
            $mform->addElement('select', 'filter', get_string('filter', 'sepl'), $options, $dirtyclass);
        }
        if (!empty($instance['markingallocationopt'])) {
            $markingfilter = get_string('markerfilter', 'sepl');
        ...
        if (!empty($instance['markingworkflowopt'])) {
            $workflowfilter = get_string('workflowfilter', 'sepl');
            $mform->addElement('select', 'workflowfilter', $workflowfilter, $instance['markingworkflowopt'], $dirtyclass);
        }
        // Quickgrading.
        if ($instance['showquickgrading']) {
            $mform->addElement('checkbox', 'quickgrading', get_string('quickgrading', 'sepl'), '', $dirtyclass);
            $mform->addHelpButton('quickgrading', 'quickgrading', 'sepl');
        ...
        // Buttons.
        $this->add_action_buttons(false, get_string('updatetable', 'sepl'));
    }
}
