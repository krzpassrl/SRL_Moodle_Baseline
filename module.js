...
M.mod_sepl.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_sepl.init_grading_table = function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.util.get_string('nousersselected', 'sepl'));
                    e.preventDefault();
                } else {
                    ...
                    if (action.indexOf(prefix) == 0) {
                        ...
                        confirmmessage = M.util.get_string('batchoperationconfirm' + action, 'seplfeedback_' + plugin);
                    } else {
                        confirmmessage = M.util.get_string('batchoperationconfirm' + operation.get('value'), 'sepl');
                    }
...
M.mod_sepl.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_sepl.init_grade_change = function(Y) {
    ...
    if (gradenode) {
        ...
        gradenode.on('change', function() {
            if (gradenode.get('value') != originalvalue) {
                alert(M.util.get_string('changegradewarning', 'mod_sepl'));
            }
        });
    }
};

M.mod_sepl.init_plugin_summary = function(Y, subtype, type, submissionid) {
