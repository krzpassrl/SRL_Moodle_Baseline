...
/**
 * This file contains the classes for the admin settings of the sepl module.
 *
 * @package   mod_sepl
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_admin_page_manage_sepl_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/sepl/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'sepl'),
                            $url);
    }
...
class sepl_plugin_manager {
    ...
    /**
     * Constructor for this seplment plugin manager
     * @param string $subtype - either seplsubmission or seplfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/sepl/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'sepl'),
                get_string('version'), get_string('hideshow', 'sepl'),
        ...
        $shortsubtype = substr($this->subtype, strlen('sepl'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/sepl/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'sepl'));
    }
