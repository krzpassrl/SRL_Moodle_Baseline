...
class seplsubmission_file_events_testcase extends advanced_testcase {
    ...
    /** @var stdClass $files Files that are being submitted for the seplment. */
    protected $files;
    ...
    /**
     * Setup all the various parts of an seplment activity including creating a file submission.
     */
    protected function setUp() {
        ...
        $dummy = (object) array(
            ...
            'itemid' => $this->submission->id,
            'filepath' => '/',
            'filename' => 'myseplmnent.png'
        );
        $this->fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $this->files = $fs->get_area_files($this->context->id, 'seplsubmission_file', ASSIGNSUBMISSION_FILE_FILEAREA,
    ...
    public function test_assessable_uploaded() {
        ...
        $event = reset($events);
        $this->assertInstanceOf('\seplsubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\seplsubmission_file\event\submission_created', $event);
        $this->assertEquals($this->context->id, $event->contextid);
