...
// File area for online text submission seplment.
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_submission_onlinetext extends sepl_submission_plugin {
    ...
    public function get_name() {
        return get_string('onlinetext', 'seplsubmission_onlinetext');
    }
    ...
    private function get_onlinetext_submission($submissionid) {
        ...
        return $DB->get_record('seplsubmission_onlinetext', array('submission'=>$submissionid));
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $name = get_string('wordlimit', 'seplsubmission_onlinetext');

        // Create a text box that can be enabled/disabled for onlinetext word limit.
        $wordlimitgrp = array();
        $wordlimitgrp[] = $mform->createElement('text', 'seplsubmission_onlinetext_wordlimit', '', $options);
        $wordlimitgrp[] = $mform->createElement('checkbox', 'seplsubmission_onlinetext_wordlimit_enabled',
                '', get_string('enable'));
        $mform->addGroup($wordlimitgrp, 'seplsubmission_onlinetext_wordlimit_group', $name, ' ', false);
        $mform->addHelpButton('seplsubmission_onlinetext_wordlimit_group',
                              'wordlimit',
                              'seplsubmission_onlinetext');
        $mform->disabledIf('seplsubmission_onlinetext_wordlimit',
                           'seplsubmission_onlinetext_wordlimit_enabled',
                           'notchecked');

        // Add numeric rule to text field.
        $wordlimitgrprules = array();
        $wordlimitgrprules['seplsubmission_onlinetext_wordlimit'][] = array(null, 'numeric', null, 'client');
        $mform->addGroupRule('seplsubmission_onlinetext_wordlimit_group', $wordlimitgrprules);

        // Rest of group setup.
        $mform->setDefault('seplsubmission_onlinetext_wordlimit', $defaultwordlimit);
        $mform->setDefault('seplsubmission_onlinetext_wordlimit_enabled', $defaultwordlimitenabled);
        $mform->setType('seplsubmission_onlinetext_wordlimit', PARAM_INT);
        $mform->disabledIf('seplsubmission_onlinetext_wordlimit_group',
                           'seplsubmission_onlinetext_enabled',
                           'notchecked');
    }
    ...
    public function save_settings(stdClass $data) {
        if (empty($data->seplsubmission_onlinetext_wordlimit) || empty($data->seplsubmission_onlinetext_wordlimit_enabled)) {
            $wordlimit = 0;
            $wordlimitenabled = 0;
        } else {
            $wordlimit = $data->seplsubmission_onlinetext_wordlimit;
            $wordlimitenabled = 1;
        }
    ...
    private function get_edit_options() {
         $editoroptions = array(
           ...
           'maxbytes' => $this->seplment->get_course()->maxbytes,
           'context' => $this->seplment->get_context(),
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        $event = \seplsubmission_onlinetext\event\assessable_uploaded::create($params);
        $event->trigger();
        ...
        if ($onlinetextsubmission) {
            ...
            $updatestatus = $DB->update_record('seplsubmission_onlinetext', $onlinetextsubmission);
            $event = \seplsubmission_onlinetext\event\submission_updated::create($params);
            ...
            $onlinetextsubmission->seplment = $this->seplment->get_instance()->id;
            $onlinetextsubmission->id = $DB->insert_record('seplsubmission_onlinetext', $onlinetextsubmission);
            $params['objectid'] = $onlinetextsubmission->id;
            $event = \seplsubmission_onlinetext\event\submission_created::create($params);
            $event->set_sepl($this->seplment);
            $event->trigger();
    ...
    public function get_editor_fields() {
        return array('onlinetext' => get_string('pluginname', 'seplsubmission_comments'));
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($onlinetextsubmission) {
            $text = $this->seplment->render_editor_content(ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
            ...
            if (!empty($CFG->enableplagiarism)) {
                ...
                $plagiarismlinks .= plagiarism_get_links(array('userid' => $submission->userid,
                    'content' => trim($text),
                    'cmid' => $this->seplment->get_course_module()->id,
                    'course' => $this->seplment->get_course()->id,
                    'seplment' => $submission->seplment));
            }
            if ($text != $shorttext) {
                $wordcount = get_string('numwords', 'seplsubmission_onlinetext', count_words($text));
    ...
    public function get_files(stdClass $submission, stdClass $user) {
        ...
        if ($onlinetextsubmission) {
            $finaltext = $this->seplment->download_rewrite_pluginfile_urls($onlinetextsubmission->onlinetext, $user, $this);
            $formattedtext = format_text($finaltext,
                                         $onlinetextsubmission->onlineformat,
                                         array('context'=>$this->seplment->get_context()));
            $head = '<head><meta charset="UTF-8"></head>';
            $submissioncontent = '<!DOCTYPE html><html>' . $head . '<body>'. $formattedtext . '</body></html>';

            $filename = get_string('onlinetextfilename', 'seplsubmission_onlinetext');
            $files[$filename] = array($submissioncontent);

            $fs = get_file_storage();

            $fsfiles = $fs->get_area_files($this->seplment->get_context()->id,
                                           'seplsubmission_onlinetext',
                                           ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                           $submission->id,
                                           'timemodified',
    ...
    public function view(stdClass $submission) {
        ...
        if ($onlinetextsubmission) {
            ...
            $result .= $this->seplment->render_editor_content(ASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                                                $onlinetextsubmission->submission,
                                                                $this->get_type(),
                                                                'onlinetext',
                                                                'seplsubmission_onlinetext');
    ...
    /**
     * Return true if this plugin can upgrade an old Moodle 2.2 seplment of this type and version.
     *
     * @param string $type old seplment subtype
     * @param int $version old seplment version
    ...
    /**
     * Upgrade the settings from the old seplment to the new plugin based one
     *
     * @param context $oldcontext - the database for the old seplment context
     * @param stdClass $oldseplment - the database for the old seplment instance
     ...
     *
     * @param context $oldcontext - the database for the old seplment context
     * @param stdClass $oldseplment The data record for the old seplment
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('seplsubmission_onlinetext', $onlinetextsubmission) > 0) {
    ...
    public function delete_instance() {
        global $DB;
        $DB->delete_records('seplsubmission_onlinetext',
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'seplsubmission_onlinetext',
        ...
        // Copy the seplsubmission_onlinetext record.
    ...
    public function check_word_count($submissiontext) {
        ...
        if ($wordcount <= $wordlimit) {
            ...
            $errormsg = get_string('wordlimitexceeded', 'seplsubmission_onlinetext',
