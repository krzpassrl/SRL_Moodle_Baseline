 ...
 * @return bool
 */
function seplsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($submission->userid) {
        $canview = $seplment->can_view_submission($submission->userid);
    } else {
        $canview = $seplment->can_view_group_submission($submission->groupid);
    }
    if (!$canview) {
...
function seplsubmission_comments_comment_permissions(stdClass $options) {
    ...
    if ($seplment->get_instance()->teamsubmission &&
        !$seplment->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$seplment->get_instance()->teamsubmission &&
        !$seplment->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
    ...
    if ($seplment->is_blind_marking() && !empty($comments)) {
        ...
        foreach ($comments as $comment) {
            ...
            if (empty($usermappings[$comment->userid])) {
                ...
                $usermappings[$comment->userid]->avatar = $seplment->get_renderer()->user_picture($commenter,
 ...
 */
function seplsubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('sepl_submission', array('id' => $submissionid));
