...
/**
 * Contains the event tests for the module sepl.
 *
 * @package   mod_sepl
...
require_once($CFG->dirroot . '/mod/sepl/tests/fixtures/event_mod_sepl_fixtures.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sepl_events_testcase extends mod_sepl_base_testcase {
    ...
    public function test_base_event() {
        ...
        /** @var \mod_sepl_unittests\event\nothing_happened $event */
        $event = \mod_sepl_unittests\event\nothing_happened::create($data);
        $sepl = $event->get_sepl();
        $this->assertDebuggingCalled();
        $this->assertInstanceOf('sepl', $sepl);

        $event = \mod_sepl_unittests\event\nothing_happened::create($data);
        $event->set_sepl($sepl);
        $sepl2 = $event->get_sepl();
        $this->assertDebuggingNotCalled();
        $this->assertSame($sepl, $sepl2);
    }
    ...
    public function test_submission_created() {
        ...
        $event = \mod_sepl_unittests\event\submission_created::create($eventinfo);
        $event->trigger();
        ...
        try {
            \mod_sepl_unittests\event\submission_created::create($params);
        ...
        try {
            \mod_sepl_unittests\event\submission_created::create($subinfo);
    ...
    public function test_submission_updated() {
        ...
        $sink = $this->redirectEvents();
        $event = \mod_sepl_unittests\event\submission_updated::create($eventinfo);
        $event->trigger();
        $result = $sink->get_events();
        $event = reset($result);
        $sink->close();
    ...
    public function test_extension_granted() {
        ...
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\extension_granted', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'grant extension',
            'view.php?id=' . $sepl->get_course_module()->id,
            $this->students[0]->id,
            $sepl->get_course_module()->id
        );
    ...
    public function test_submission_locked() {
        ...
        $sepl->lock_submission($this->students[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\submission_locked', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            ...
            'lock submission',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('locksubmissionforstudent', 'sepl', array('id' => $this->students[0]->id,
    ...
    public function test_identities_revealed() {
        ...
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\identities_revealed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $expected = array(
            $sepl->get_course()->id,
            ...
            'reveal identities',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('revealidentities', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_status_viewed() {
        ...
        $this->assertInstanceOf('\mod_sepl\event\submission_status_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'view',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewownsubmissionstatus', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_submission_status_updated() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\mod_sepl\event\submission_status_updated', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('reverttodraftforstudent', 'sepl', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_marker_updated() {
        ...
        $sink = $this->redirectEvents();
        $sepl->testable_process_set_batch_marking_allocation($this->students[0]->id, $this->teachers[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\marker_updated', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        ...
        $expected = array(
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('setmarkerallocationforlog', 'sepl', array('id' => $this->students[0]->id,
    ...
    public function test_workflow_state_updated() {
        ...
        $sink = $this->redirectEvents();
        $sepl->testable_process_set_batch_marking_workflow_state($this->students[0]->id, ASSIGN_MARKING_WORKFLOW_STATE_INREVIEW);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\workflow_state_updated', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $this->assertEquals($this->editingteachers[0]->id, $event->userid);
        ...
        $expected = array(
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('setmarkingworkflowstateforlog', 'sepl', array('id' => $this->students[0]->id,
        ...
        $sepl->testable_process_save_quick_grades($data);
    ...
    public function test_submission_duplicated() {
        ...
        $sepl = $this->create_instance();
        $submission1 = $sepl->get_user_submission($this->students[0]->id, true, 0);
        $submission2 = $sepl->get_user_submission($this->students[0]->id, true, 1);
        $submission2->status = ASSIGN_SUBMISSION_STATUS_REOPENED;
        $sepl->testable_update_submission($submission2, $this->students[0]->id, time(), $sepl->get_instance()->teamsubmission);

        $sink = $this->redirectEvents();
        $notices = null;
        $sepl->copy_previous_attempt($notices);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\submission_duplicated', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'submissioncopied',
            'view.php?id=' . $sepl->get_course_module()->id,
            $sepl->testable_format_submission_for_log($submission2),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
    ...
    public function test_submission_unlocked() {
        ...
        $sepl->unlock_submission($this->students[0]->id);

        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);
        $this->assertInstanceOf('\mod_sepl\event\submission_unlocked', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($sepl->get_instance()->id, $event->objectid);
        $this->assertEquals($this->students[0]->id, $event->relateduserid);
        $expected = array(
            ...
            'unlock submission',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('unlocksubmissionforstudent', 'sepl', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_submission_graded() {
        ...
        $data->grade = '50.0';
        $sepl->testable_apply_grade_to_user($data, $this->students[0]->id, 0);
        $grade = $sepl->get_user_grade($this->students[0]->id, false, 0);

        $events = $sink->get_events();
        $this->assertCount(3, $events);
        $event = $events[2];
        $this->assertInstanceOf('\mod_sepl\event\submission_graded', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'grade submission',
            'view.php?id=' . $sepl->get_course_module()->id,
            $sepl->format_grade_for_log($grade),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $sink->close();
        ...
        $sepl->testable_process_save_quick_grades($data);
        $grade = $sepl->get_user_grade($this->students[0]->id, false);
    ...
    public function test_submission_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->view('viewpluginseplsubmission');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\submission_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $this->assertEquals($submission->id, $event->objectid);
        $expected = array(
            $sepl->get_course()->id,
            ...
            'view submission',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewsubmissionforuser', 'sepl', $this->students[0]->id),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_feedback_viewed() {
        ...
        $grade = new stdClass();
        $grade->seplment = $sepl->get_instance()->id;
        $grade->userid = $this->students[0]->id;
        $gradeid = $DB->insert_record('sepl_grades', $grade);
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->view('viewpluginseplfeedback');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\feedback_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        ...
        $expected = array(
            ...
            'view feedback',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewfeedbackforuser', 'sepl', $this->students[0]->id),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_grading_form_viewed() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\grading_form_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'view grading form',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewgradingformforstudent', 'sepl', array('id' => $this->students[0]->id,
                'fullname' => fullname($this->students[0]))),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_grading_table_viewed() {
        ...
        $expected = array(
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewsubmissiongradingtable', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_submission_form_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->view('editsubmission');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\submission_form_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'view submit seplment form',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('editsubmission', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_submission_confirmation_form_viewed() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\submission_confirmation_form_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'view confirm submit seplment form',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewownsubmissionform', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_reveal_identities_confirmation_page_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->view('revealidentities');
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\reveal_identities_confirmation_page_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            'view',
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewrevealidentitiesconfirm', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_statement_accepted() {
        // We want to be a student so we can submit seplments.
        $this->setUser($this->students[0]);

        // We do not want to send any messages to the student during the PHPUNIT test.
        set_config('submissionreceipts', false, 'sepl');
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->submit_for_grading($data, array());
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\statement_accepted', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
    ...
    public function test_batch_set_workflow_state_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->testable_view_batch_set_workflow_state($this->students[0]->id);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\batch_set_workflow_state_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewbatchsetmarkingworkflowstate', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_batch_set_marker_allocation_viewed() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        $sepl->testable_view_batch_markingallocation($this->students[0]->id);
        $events = $sink->get_events();
        $event = reset($events);

        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_sepl\event\batch_set_marker_allocation_viewed', $event);
        $this->assertEquals($sepl->get_context(), $event->get_context());
        $expected = array(
            $sepl->get_course()->id,
            'sepl',
            ...
            'view.php?id=' . $sepl->get_course_module()->id,
            get_string('viewbatchmarkingallocation', 'sepl'),
            $sepl->get_course_module()->id
        );
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
