...
/**
 * Unit tests for (some of) mod/sepl/locallib.php.
 *
 * @package    mod_sepl
 * @category   phpunit
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sepl_locallib_testcase extends mod_sepl_base_testcase {
    ...
    public function test_return_links() {
        ...
        $sepl = $this->create_instance();
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));
        $sepl->register_return_link($returnaction, $returnparams);
        $this->assertEquals($returnaction, $sepl->get_return_action());
        $this->assertEquals($returnparams, $sepl->get_return_params());
    }

    public function test_get_feedback_plugins() {
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance();
        $installedplugins = array_keys(core_component::get_plugin_list('seplfeedback'));

        foreach ($sepl->get_feedback_plugins() as $plugin) {
    ...
    public function test_get_submission_plugins() {
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance();
        $installedplugins = array_keys(core_component::get_plugin_list('seplsubmission'));

        foreach ($sepl->get_submission_plugins() as $plugin) {
    ...
    public function test_is_blind_marking() {
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('blindmarking'=>1));
        $this->assertEquals(true, $sepl->is_blind_marking());

        // Test cannot see student names.
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, get_string('hiddenuser', 'sepl')));
        ...
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertEquals(false, strpos($output, get_string('hiddenuser', 'sepl')));
    ...
    public function test_gradingtable_extension_due_date() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array(
            'seplsubmission_onlinetext_enabled'=>1,
            'duedate' => time() - 4 * 24 * 60 * 60,
         ));
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array(
            'id' => $sepl->get_course_module()->id,
            'action' => 'grading',
        )));

        // Check that the seplment is late.
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertContains(get_string('submissionstatus_', 'sepl'), $output);
        $this->assertContains(get_string('overdue', 'sepl', format_time(4*24*60*60)), $output);

        // Grant an extension.
        $extendedtime = time() + 2 * 24 * 60 * 60;
        $sepl->testable_save_user_extension($this->students[0]->id, $extendedtime);
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertContains(get_string('submissionstatus_', 'sepl'), $output);
        $this->assertContains(get_string('userextensiondate', 'sepl', userdate($extendedtime)), $output);

        // Simulate a submission.
        $this->setUser($this->students[0]);
        $submission = $sepl->get_user_submission($this->students[0]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->students[0]->id, true, false);
        $data = new stdClass();
        ...
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertContains(get_string('submissionstatus_submitted', 'sepl'), $output);
        $this->assertContains(get_string('userextensiondate', 'sepl', userdate($extendedtime)), $output);
    }
    ...
    public function test_gradingtable_extension_date_calculation_for_lateness() {
        ...
        $this->assertContains(get_string('submissionstatus_', 'sepl'), $output);
        $difftime = time() - $time;
        $this->assertContains(get_string('overdue', 'sepl', format_time(4*24*60*60 + $difftime)), $output);

        // Grant an extension that is in the past.
        $sepl->testable_save_user_extension($this->students[0]->id, $time - 2 * 24 * 60 * 60);
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertContains(get_string('submissionstatus_', 'sepl'), $output);
        $this->assertContains(get_string('userextensiondate', 'sepl', userdate($time - 2*24*60*60)), $output);
        $difftime = time() - $time;
        $this->assertContains(get_string('overdue', 'sepl', format_time(2*24*60*60 + $difftime)), $output);
        ...
        $this->assertContains(get_string('submittedlateshort', 'sepl', format_time(2*24*60*60 + $difftime)), $output);
    }
    ...
    public function test_gradingtable_group_submissions_rendering() {
        ...
        $submission = $sepl->get_group_submission($this->extrastudents[0]->id, 0, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->extrastudents[0]->id, true, true);

        // Check output.
        $this->setUser($this->teachers[0]);
        $gradingtable = new sepl_grading_table($sepl, 4, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        ...
        // Check status.
        $this->assertSame(get_string('submissionstatus_submitted', 'sepl'), $xpath->evaluate('string(//td[@id="mod_sepl_grading_r0_c4"]/div[@class="submissionstatussubmitted"])'));
        $this->assertSame(get_string('submissionstatus_submitted', 'sepl'), $xpath->evaluate('string(//td[@id="mod_sepl_grading_r3_c4"]/div[@class="submissionstatussubmitted"])'));

        // Check submission last modified date
        $this->assertGreaterThan(0, strtotime($xpath->evaluate('string(//td[@id="mod_sepl_grading_r0_c8"])')));
        $this->assertGreaterThan(0, strtotime($xpath->evaluate('string(//td[@id="mod_sepl_grading_r3_c8"])')));

        // Check group.
        $this->assertSame($this->groups[0]->name, $xpath->evaluate('string(//td[@id="mod_sepl_grading_r0_c5"])'));
        $this->assertSame($this->groups[0]->name, $xpath->evaluate('string(//td[@id="mod_sepl_grading_r3_c5"])'));

        // Check submission text.
        $this->assertSame('Submission text', $xpath->evaluate('string(//td[@id="mod_sepl_grading_r0_c9"]/div/div)'));
        $this->assertSame('Submission text', $xpath->evaluate('string(//td[@id="mod_sepl_grading_r3_c9"]/div/div)'));

        // Check comments can be made.
        $this->assertSame(1, (int)$xpath->evaluate('count(//td[@id="mod_sepl_grading_r0_c10"]//textarea)'));
        $this->assertSame(1, (int)$xpath->evaluate('count(//td[@id="mod_sepl_grading_r3_c10"]//textarea)'));
    }

    public function test_show_intro() {
        // Test whether we are showing the intro at the correct times.
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('alwaysshowdescription'=>1));

        $this->assertEquals(true, $sepl->testable_show_intro());

        $tomorrow = time() + (24*60*60);

        $sepl = $this->create_instance(array('alwaysshowdescription'=>0,
                                               'allowsubmissionsfromdate'=>$tomorrow));
        $this->assertEquals(false, $sepl->testable_show_intro());
    ...
    public function test_has_submissions_or_grades() {
        ...
        $instance = $sepl->get_instance();

        // Should start empty.
        $this->assertEquals(false, $sepl->has_submissions_or_grades());
        ...
        // Now test again.
        $this->assertEquals(true, $sepl->has_submissions_or_grades());
        // Set this back to default.
        $this->students[0]->ignoresesskey = false;
    }
    ...
    public function test_delete_grades() {
        ...
        $gradinginfo = grade_get_grades($this->course->id,
                                        'mod',
                                        'sepl',
                                        $sepl->get_instance()->id);

        $this->assertNotEquals(0, count($gradinginfo->items));

        $sepl->testable_delete_grades();
        $gradinginfo = grade_get_grades($this->course->id,
                                        'mod',
                                        'sepl',
                                        $sepl->get_instance()->id);
    ...
    public function test_delete_instance() {
        ...
        // Now try and delete.
        $this->assertEquals(true, $sepl->delete_instance());
    }
    ...
    public function test_reset_userdata() {
        ...
        // Now try and reset.
        $data = new stdClass();
        $data->reset_sepl_submissions = 1;
        $data->reset_gradebook_grades = 1;
        $data->courseid = $this->course->id;
        $data->timeshift = 24*60*60;
        $this->setUser($this->editingteachers[0]);
        $sepl->reset_userdata($data);
        $this->assertEquals(false, $sepl->has_submissions_or_grades());

        // Reload the instance data.
        $instance = $DB->get_record('sepl', array('id'=>$sepl->get_instance()->id));
        $this->assertEquals($now + 24*60*60, $instance->duedate);

        // Test reset using sepl_reset_userdata().
        $seplduedate = $instance->duedate; // Keep old updated value for comparison.
        $data->timeshift = 2*24*60*60;
        sepl_reset_userdata($data);
        $instance = $DB->get_record('sepl', array('id' => $sepl->get_instance()->id));
        $this->assertEquals($seplduedate + 2*24*60*60, $instance->duedate);

        // Create one more seplment and reset, make sure time shifted for previous seplment is not changed.
        $sepl2 = $this->create_instance(array('seplsubmission_onlinetext_enabled' => 1,
                                               'duedate' => $now));
        $seplduedate = $instance->duedate;
        $data->timeshift = 3*24*60*60;
        $sepl2->reset_userdata($data);
        $instance = $DB->get_record('sepl', array('id' => $sepl->get_instance()->id));
        $this->assertEquals($seplduedate, $instance->duedate);
        $instance2 = $DB->get_record('sepl', array('id' => $sepl2->get_instance()->id));
        $this->assertEquals($now + 3*24*60*60, $instance2->duedate);

        // Reset both seplments using sepl_reset_userdata() and make sure both seplments have same date.
        $seplduedate = $instance->duedate;
        $sepl2duedate = $instance2->duedate;
        $data->timeshift = 4*24*60*60;
        sepl_reset_userdata($data);
        $instance = $DB->get_record('sepl', array('id' => $sepl->get_instance()->id));
        $this->assertEquals($seplduedate + 4*24*60*60, $instance->duedate);
        $instance2 = $DB->get_record('sepl', array('id' => $sepl2->get_instance()->id));
        $this->assertEquals($sepl2duedate + 4*24*60*60, $instance2->duedate);
    }
    ...
    public function test_plugin_settings() {
        ...
        $now = time();
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('seplsubmission_file_enabled'=>1,
                                               'seplsubmission_file_maxfiles'=>12,
                                               'seplsubmission_file_maxsizebytes'=>10));
    ...
    public function test_update_calendar() {
        ...
        // Create a new seplment with links to a draft area.
        $now = time();
        $sepl = $this->create_instance(array(
            'duedate' => $now,
            'intro' => $fakearealink1,
        ...
        $params = array('modulename'=>'sepl', 'instance'=>$sepl->get_instance()->id);
        $event = $DB->get_record('event', $params);
        $this->assertNotEmpty($event);
        $this->assertSame('link', $event->description);     // The pluginfile links are removed.

        // Make sure the same works when updating the seplment.
        $instance = $sepl->get_instance();
        $instance->instance = $instance->id;
        $instance->intro = $fakearealink2;
        $instance->introformat = FORMAT_HTML;
        $sepl->update_instance($instance);
        $params = array('modulename' => 'sepl', 'instance' => $sepl->get_instance()->id);
        $event = $DB->get_record('event', $params);
        $this->assertNotEmpty($event);
        $this->assertSame('new', $event->description);     // The pluginfile links are removed.

        // Create an seplment with a description that should be hidden.
        $sepl = $this->create_instance(array('duedate'=>$now + 160,
        ...
        // because if we call the seplment update method - it will update the calendar
        // and we want to test that this works from cron.
        $DB->set_field('sepl', 'allowsubmissionsfromdate', $now - 60, array('id'=>$sepl->get_instance()->id));
    ...
    public function test_update_instance() {
        ...
        $instance->instance = $instance->id;
        $instance->seplsubmission_onlinetext_enabled = 1;
    ...
    public function test_cannot_submit_empty() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('submissiondrafts'=>1));

        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));

        // Test you cannot see the submit button for an offline seplment regardless.
        $this->setUser($this->students[0]);
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertNotContains(get_string('submitseplment', 'sepl'), $output, 'Can submit empty offline seplment');

        // Test you cannot see the submit button for an online text seplment with no submission.
        $this->setUser($this->editingteachers[0]);
        $instance = $sepl->get_instance();
        $instance->instance = $instance->id;
        $instance->seplsubmission_onlinetext_enabled = 1;
        ...
        $this->setUser($this->students[0]);
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertNotContains(get_string('submitseplment', 'sepl'), $output, 'Cannot submit empty onlinetext seplment');
        ...
        $plugin = $sepl->get_submission_plugin_by_type('onlinetext');
        $plugin->save($submission, $data);
        // Test you can see the submit button for an online text seplment with a submission.
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertContains(get_string('submitseplment', 'sepl'), $output, 'Can submit non empty onlinetext seplment');
    }
    ...
    public function test_list_participants() {
        ...
        $this->create_extra_users();
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('grade'=>100));

        $this->assertEquals(self::DEFAULT_STUDENT_COUNT + self::EXTRA_STUDENT_COUNT, count($sepl->list_participants(null, true)));
        ...
        $this->assertEquals(self::DEFAULT_STUDENT_COUNT + self::EXTRA_STUDENT_COUNT + self::EXTRA_SUSPENDED_COUNT,
                count($sepl->list_participants(null, true)));
        ...
        $this->assertEquals(2, count($sepl->list_participants(null, true)));
    }

    public function test_count_teams() {
        $this->create_extra_users();
        $this->setUser($this->editingteachers[0]);
        $sepl1 = $this->create_instance(array('teamsubmission' => 1));
        $this->assertEquals(self::GROUP_COUNT + 1, $sepl1->count_teams());
        ...
        $sepl2 = $this->create_instance($params);
        $this->assertEquals(3, $sepl2->count_teams());

        // An active group => Just the selected one.
        $this->assertEquals(1, $sepl2->count_teams($this->groups[0]->id));
        ...
        $sepl3 = $this->create_instance($params);
        $this->assertEquals(2, $sepl3->count_teams());

        $sepl4 = $this->create_instance(array('teamsubmission' => 1, 'preventsubmissionnotingroup' => true));
        $this->assertEquals(self::GROUP_COUNT, $sepl4->count_teams());
    }
    ...
    public function test_submit_to_default_group() {
        ...
        $sepl = $this->create_instance($params);
        ...
        $group = $sepl->get_submission_group($newstudent->id);
        ...
        $this->assertEquals(1, $sepl->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));

        // Set an active group.
        $anothergroup = $this->groups[0];
        $SESSION->activegroup[$this->course->id]['aag'][0] = (int)$anothergroup->id;
        $this->assertEquals(0, $sepl->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));
    ...
    public function test_count_submissions() {
        ...
        $this->create_extra_users();
        $this->setUser($this->editingteachers[0]);
        $sepl1 = $this->create_instance(array('seplsubmission_onlinetext_enabled' => 1));

        // Simulate a submission.
        $this->setUser($this->extrastudents[0]);
        $submission = $sepl1->get_user_submission($this->extrastudents[0]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_DRAFT;
        $sepl1->testable_update_submission($submission, $this->extrastudents[0]->id, true, false);
        // Leave this one as DRAFT.
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
        $plugin = $sepl1->get_submission_plugin_by_type('onlinetext');
        $plugin->save($submission, $data);
        ...
        $data = new stdClass();
        $data->grade = '50.0';
        $sepl1->testable_apply_grade_to_user($data, $this->extrastudents[0]->id, 0);

        // Simulate a submission.
        $this->setUser($this->extrastudents[1]);
        $submission = $sepl1->get_user_submission($this->extrastudents[1]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl1->testable_update_submission($submission, $this->extrastudents[1]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
        ...
        $submission = $sepl1->get_user_submission($this->extrastudents[2]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl1->testable_update_submission($submission, $this->extrastudents[2]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
        ...
        $submission = $sepl1->get_user_submission($this->extrastudents[3]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl1->testable_update_submission($submission, $this->extrastudents[3]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
        ...
        $this->setUser($this->extrastudents[3]);
        $submission = $sepl1->get_user_submission($this->extrasuspendedstudents[0]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl1->testable_update_submission($submission, $this->extrasuspendedstudents[0]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
        ...
        $data = new stdClass();
        $data->grade = '50.0';
        $sepl1->testable_apply_grade_to_user($data, $this->extrastudents[3]->id, 0);
        $sepl1->testable_apply_grade_to_user($data, $this->extrasuspendedstudents[0]->id, 0);

        // Create a new submission with status NEW.
        $this->setUser($this->extrastudents[4]);
        $submission = $sepl1->get_user_submission($this->extrastudents[4]->id, true);

        $this->assertEquals(2, $sepl1->count_grades());
        $this->assertEquals(4, $sepl1->count_submissions());
        $this->assertEquals(5, $sepl1->count_submissions(true));
        $this->assertEquals(2, $sepl1->count_submissions_need_grading());
        $this->assertEquals(3, $sepl1->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(1, $sepl1->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_DRAFT));

        // Groups.
        $sepl2 = $this->create_instance(array(
            'seplsubmission_onlinetext_enabled' => 1,
            'groupmode' => VISIBLEGROUPS
        ));

        $this->setUser($this->extrastudents[1]);
        $submission = $sepl2->get_user_submission($this->extrastudents[1]->id, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl2->testable_update_submission($submission, $this->extrastudents[1]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        $plugin = $sepl2->get_submission_plugin_by_type('onlinetext');
        $plugin->save($submission, $data);

        $this->assertEquals(1, $sepl2->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));
        ...
        $SESSION->activegroup[$this->course->id]['aag'][0] = (int)$anothergroup->id;
        $this->assertEquals(0, $sepl2->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));
    }
    ...
    public function test_count_submissions_for_groups() {
        ...
        $groupid = null;
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('seplsubmission_onlinetext_enabled' => 1, 'teamsubmission' => 1));

        // Simulate a submission.
        $this->setUser($this->extrastudents[0]);
        $submission = $sepl->get_group_submission($this->extrastudents[0]->id, $groupid, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_DRAFT;
        $sepl->testable_update_submission($submission, $this->extrastudents[0]->id, true, false);
        // Leave this one as DRAFT.
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
        ...
        $data = new stdClass();
        $data->grade = '50.0';
        $sepl->testable_apply_grade_to_user($data, $this->extrastudents[0]->id, 0);

        // Simulate a submission.
        $this->setUser($this->extrastudents[1]);
        $submission = $sepl->get_group_submission($this->extrastudents[1]->id, $groupid, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->extrastudents[1]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        // Simulate a submission.
        $this->setUser($this->extrastudents[2]);
        $submission = $sepl->get_group_submission($this->extrastudents[2]->id, $groupid, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->extrastudents[2]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        // Simulate a submission.
        $this->setUser($this->extrastudents[3]);
        $submission = $sepl->get_group_submission($this->extrastudents[3]->id, $groupid, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->extrastudents[3]->id, true, false);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                                         'text' => 'Submission text',
                                         'format' => FORMAT_MOODLE);
        ...
        $data = new stdClass();
        $data->grade = '50.0';
        $sepl->testable_apply_grade_to_user($data, $this->extrastudents[3]->id, 0);
        $sepl->testable_apply_grade_to_user($data, $this->extrasuspendedstudents[0]->id, 0);

        // Create a new submission with status NEW.
        $this->setUser($this->extrastudents[4]);
        $submission = $sepl->get_group_submission($this->extrastudents[4]->id, $groupid, true);

        $this->assertEquals(2, $sepl->count_grades());
        $this->assertEquals(4, $sepl->count_submissions());
        $this->assertEquals(5, $sepl->count_submissions(true));
        $this->assertEquals(3, $sepl->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_SUBMITTED));
        $this->assertEquals(1, $sepl->count_submissions_with_status(ASSIGN_SUBMISSION_STATUS_DRAFT));
    }
    ...
    public function test_get_grading_userid_list() {
        ...
        $users = $sepl->testable_get_grading_userid_list();
    ...
    public function test_cron() {
        ...
        // Now create an seplment and add some feedback.
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('sendstudentnotifications'=>1));
        ...
        $sepl->testable_apply_grade_to_user($data, $this->students[2]->id, 0);

        // Now run cron and see that one message was sent.
        $this->preventResetByRollback();
        $sink = $this->redirectMessages();
        cron_setup_user();
        $this->expectOutputRegex('/Done processing 2 seplment submissions/');
        sepl::cron();
    ...
    public function test_is_graded() {
        ...
        $this->assertEquals(true, $sepl->testable_is_graded($this->students[0]->id));
        $this->assertEquals(false, $sepl->testable_is_graded($this->students[1]->id));
    }
    ...
    public function test_can_grade() {
        ...
        $this->setUser($this->students[0]);
        $this->assertEquals(false, $sepl->can_grade());
        $this->setUser($this->editingteachers[0]);
        $this->assertEquals(true, $sepl->can_grade());
        $this->setUser($this->teachers[0]);
        $this->assertEquals(true, $sepl->can_grade());

        // Test the viewgrades capability - without mod/sepl:grade.
        $this->setUser($this->students[0]);
        $studentrole = $DB->get_record('role', array('shortname' => 'student'));
        sepl_capability('mod/sepl:viewgrades', CAP_ALLOW, $studentrole->id, $sepl->get_context()->id);
        $this->assertEquals(false, $sepl->can_grade());
    }
    ...
    public function test_can_view_submission() {
        ...
        $this->setUser($this->students[0]);
        $this->assertEquals(true, $sepl->can_view_submission($this->students[0]->id));
        $this->assertEquals(false, $sepl->can_view_submission($this->students[1]->id));
        $this->assertEquals(false, $sepl->can_view_submission($this->teachers[0]->id));
        $this->setUser($this->teachers[0]);
        $this->assertEquals(true, $sepl->can_view_submission($this->students[0]->id));
        $this->assertEquals(true, $sepl->can_view_submission($this->students[1]->id));
        $this->assertEquals(true, $sepl->can_view_submission($this->teachers[0]->id));
        $this->assertEquals(false, $sepl->can_view_submission($this->extrasuspendedstudents[0]->id));
        $this->setUser($this->editingteachers[0]);
        $this->assertEquals(true, $sepl->can_view_submission($this->students[0]->id));
        $this->assertEquals(true, $sepl->can_view_submission($this->students[1]->id));
        $this->assertEquals(true, $sepl->can_view_submission($this->teachers[0]->id));
        $this->assertEquals(true, $sepl->can_view_submission($this->extrasuspendedstudents[0]->id));
    ...
    public function test_update_submission() {
        ...
        $this->setUser($this->extrastudents[0]);
        $now = time();
        $submission = $sepl->get_user_submission($this->extrastudents[0]->id, true);
        $sepl->testable_update_submission($submission, $this->extrastudents[0]->id, true, false);
        ...
        // Now verify group seplments.
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('teamsubmission'=>1));
        ...
        $this->setUser($this->extrastudents[1]);
        $now = time();
        $submission = $sepl->get_group_submission($this->extrastudents[1]->id, 0, true);
        $sepl->testable_update_submission($submission, $this->extrastudents[1]->id, true, true);
    ...
    public function test_group_submissions_submit_for_marking_requireallteammemberssubmit() {
        ...
        $output = $sepl->view_student_summary($this->extrastudents[0], true);
        $this->assertContains(get_string('submitseplment', 'sepl'), $output);
        ...
        $output = $sepl->view_student_summary($this->extrastudents[0], true);
        $this->assertNotContains(get_string('submitseplment', 'sepl'), $output);

        // Change to another user in the same group.
        $this->setUser($this->extrastudents[self::GROUP_COUNT]);
        $output = $sepl->view_student_summary($this->extrastudents[self::GROUP_COUNT], true);
        $this->assertContains(get_string('submitseplment', 'sepl'), $output);

        $submission = $sepl->get_group_submission($this->extrastudents[self::GROUP_COUNT]->id, 0, true);
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->extrastudents[self::GROUP_COUNT]->id, true, true);
        $output = $sepl->view_student_summary($this->extrastudents[self::GROUP_COUNT], true);
        $this->assertNotContains(get_string('submitseplment', 'sepl'), $output);
    }
    ...
    public function test_group_submissions_submit_for_marking() {
        ...
        $output = $sepl->view_student_summary($this->extrastudents[0], true);
        $this->assertContains(get_string('submitseplment', 'sepl'), $output);
        $this->assertContains(get_string('timeremaining', 'sepl'), $output);
        $difftime = time() - $time;
        $this->assertContains(get_string('overdue', 'sepl', format_time(2*24*60*60 + $difftime)), $output);
        ...
        $this->assertContains(get_string('timeremaining', 'sepl'), $output);
        $difftime = time() - $time;
        $this->assertContains(get_string('submittedlate', 'sepl', format_time(2*24*60*60 + $difftime)), $output);
    ...
    public function test_submissions_open() {
        ...
        $sepl = $this->create_instance();
        $this->assertEquals(true, $sepl->testable_submissions_open($this->students[0]->id));

        $sepl = $this->create_instance(array('duedate'=>$tomorrow));
        $this->assertEquals(true, $sepl->testable_submissions_open($this->students[0]->id));

        $sepl = $this->create_instance(array('duedate'=>$yesterday));
        $this->assertEquals(true, $sepl->testable_submissions_open($this->students[0]->id));

        $sepl = $this->create_instance(array('duedate'=>$yesterday, 'cutoffdate'=>$tomorrow));
        $this->assertEquals(true, $sepl->testable_submissions_open($this->students[0]->id));

        $sepl = $this->create_instance(array('duedate'=>$yesterday, 'cutoffdate'=>$yesterday));
        $this->assertEquals(false, $sepl->testable_submissions_open($this->students[0]->id));

        $sepl->testable_save_user_extension($this->students[0]->id, $tomorrow);
        $this->assertEquals(true, $sepl->testable_submissions_open($this->students[0]->id));
    ...
    public function test_get_graders() {
        ...
        // Create an seplment with no groups.
        $sepl = $this->create_instance();
        $this->assertCount(self::DEFAULT_TEACHER_COUNT +
                           ...
                           $sepl->testable_get_graders($this->students[0]->id));
        ...
        $this->assertCount(4, $sepl->testable_get_graders($this->students[0]->id));
        ...
        $this->assertCount(10, $sepl->testable_get_graders($this->students[1]->id));
    }

    public function test_get_notified_users() {
        global $CFG, $DB;

        $capability = 'mod/sepl:receivegradernotifications';
        ...
        $this->assertCount(self::DEFAULT_TEACHER_COUNT +
                           ...
                           self::EXTRA_TEACHER_COUNT +
                           self::EXTRA_EDITING_TEACHER_COUNT,
                           $sepl->testable_get_notifiable_users($this->students[0]->id));

        // Change nonediting teachers role to not receive grader notifications.
        sepl_capability($capability, CAP_PROHIBIT, $role->id, $coursecontext);
        ...
        unsepl_capability($capability, $role->id, $coursecontext);
        ...
        $this->setUser($this->students[1]);
        $this->assertCount(4, $sepl->testable_get_notifiable_users($this->students[0]->id));
        // Note the second student is in a group that is not in the grouping.
        // This means that we get all graders that are not in a group in the grouping.
        $this->assertCount(10, $sepl->testable_get_notifiable_users($this->students[1]->id));

        // Change nonediting teachers role to not receive grader notifications.
        sepl_capability($capability, CAP_PROHIBIT, $role->id, $coursecontext);

        $this->assertCount(2, $sepl->testable_get_notifiable_users($this->students[0]->id));
        // Note the second student is in a group that is not in the grouping.
        // This means that we get all graders that are not in a group in the grouping.
        $this->assertCount(5, $sepl->testable_get_notifiable_users($this->students[1]->id));
    }
    ...
    public function test_group_members_only() {
        ...
        $CFG->enableavailability = true;
        $grouping = $this->getDataGenerator()->create_grouping(array('courseid' => $this->course->id));
        groups_sepl_grouping($grouping->id, $this->groups[0]->id);

        // Force create an seplment with SEPARATEGROUPS.
        $instance = $this->getDataGenerator()->create_module('sepl', array('course'=>$this->course->id),
        ...
        $sepl = new testable_sepl($context, $cm, $this->course);

        $this->setUser($this->teachers[0]);
        get_fast_modinfo($this->course, 0, true);
        $this->assertCount(5, $sepl->list_participants(0, true));
    ...
    public function test_get_uniqueid_for_user() {
        ...
        foreach ($this->students as $student) {
            $uniqueid = $sepl->get_uniqueid_for_user($student->id);
            $this->assertEquals($student->id, $sepl->get_user_id_for_uniqueid($uniqueid));
        }
    }
    ...
    public function test_show_student_summary() {
        ...
        $gradeitem = new grade_item(array(
            'itemtype'      => 'mod',
            'itemmodule'    => 'sepl',
            'iteminstance'  => $sepl->get_instance()->id,
        ...
        $sepl = $this->create_instance(array('seplfeedback_comments_enabled' => 1));

        $this->setUser($this->teachers[0]);
        $grade = $sepl->get_user_grade($this->students[0]->id, true);
        $data = new stdClass();
        $data->seplfeedbackcomments_editor = array('text'=>'Tomato sauce',
                                         'format'=>FORMAT_MOODLE);
    ...
    public function test_attempt_reopen_method_manual() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('attemptreopenmethod'=>ASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL,
                                               'maxattempts'=>3,
                                               'submissiondrafts'=>1,
                                               'seplsubmission_onlinetext_enabled'=>1));
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));
        ...
        $this->setUser($this->students[0]);
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertNotEquals(false, strpos($output, get_string('addsubmission', 'sepl')));
        ...
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertEquals(false, strpos($output, get_string('addsubmission', 'sepl')));
        ...
        $this->teachers[0]->ignoresesskey = true;
        $this->setUser($this->teachers[0]);
        $result = $sepl->testable_process_add_attempt($this->students[0]->id);
        ...
        $this->assertNotEquals(false, strpos($output, get_string('addnewattempt', 'sepl')));
        // Check that the student now does not have a button for Submit.
        $this->assertEquals(false, strpos($output, get_string('submitseplment', 'sepl')));

        // Check that the student now has a submission history.
        $this->assertNotEquals(false, strpos($output, get_string('attempthistory', 'sepl')));
        ...
        $gradingtable = new sepl_grading_table($sepl, 100, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, $this->students[0]->lastname));

        // Should be 1 not 2.
        $this->assertEquals(1, $sepl->count_submissions());
        $this->assertEquals(1, $sepl->count_submissions_with_status('reopened'));
        $this->assertEquals(0, $sepl->count_submissions_need_grading());
        $this->assertEquals(1, $sepl->count_grades());

        // Change max attempts to unlimited.
        $formdata = clone($sepl->get_instance());
        $formdata->maxattempts = ASSIGN_UNLIMITED_ATTEMPTS;
        $formdata->instance = $formdata->id;
        $sepl->update_instance($formdata);

        // Mark the submission again.
        $data = new stdClass();
        $data->grade = '60.0';
        $sepl->testable_apply_grade_to_user($data, $this->students[0]->id, 1);

        // Check the grade exists.
        $grades = $sepl->get_user_grades_for_gradebook($this->students[0]->id);
    ...
    public function test_attempt_reopen_method_untilpass() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('attemptreopenmethod' => ASSIGN_ATTEMPT_REOPEN_METHOD_UNTILPASS,
                'maxattempts' => 3,
                'submissiondrafts' => 1,
                'seplsubmission_onlinetext_enabled' => 1));
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));
        ...
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertEquals(false, strpos($output, get_string('addnewattempt', 'sepl')));
        ...
        $now = time();
        $submission = $sepl->get_user_submission($this->students[0]->id, true, 1);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                'text' => 'Submission text',
                'format' => FORMAT_MOODLE);
        ...
        $this->setUser($this->students[1]);
        $now = time();
        $submission = $sepl->get_user_submission($this->students[1]->id, true);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                'text' => 'Submission text',
                'format' => FORMAT_MOODLE);
        ...
        // And now submit it for marking.
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->students[1]->id, true, false);
        ...
        // Check the student can see the grade.
        $this->setUser($this->students[1]);
        $output = $sepl->view_student_summary($this->students[1], true);
        ...
        $now = time();
        $submission = $sepl->get_user_submission($this->students[2]->id, true);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid' => file_get_unused_draft_itemid(),
                'text' => 'Submission text',
                'format' => FORMAT_MOODLE);
        ...
        // And now submit it for marking.
        $submission->status = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
        $sepl->testable_update_submission($submission, $this->students[2]->id, true, false);
        ...
        // Check the student can see the grade.
        $this->setUser($this->students[2]);
        $output = $sepl->view_student_summary($this->students[2], true);
    ...
    public function test_markingworkflow() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('markingworkflow'=>1));
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));
    ...
    public function test_markerallocation() {
        ...
        $this->setUser($this->editingteachers[0]);
        $sepl = $this->create_instance(array('markingworkflow'=>1, 'markingallocation'=>1));
        $PAGE->set_url(new moodle_url('/mod/sepl/view.php', array('id' => $sepl->get_course_module()->id)));
    ...
    public function test_teacher_submit_for_student() {
        ...
        $data->userid = $this->students[0]->id;
        $notices = array();
        $sepl->submit_for_grading($data, $notices);

        // Revert to draft so the student can edit it.
        $sepl->revert_to_draft($this->students[0]->id);
        ...
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertNotContains(get_string('addsubmission', 'sepl'), $output);
    ...
    public function test_disable_submit_after_cutoff_date() {
        ...
        $this->setUser($this->editingteachers[0]);
        $now = time();
        $sepl = $this->create_instance(array('duedate'=>$lastweek,
                                               'cutoffdate'=>$yesterday,
                                               'seplsubmission_onlinetext_enabled'=>1));

        $this->setUser($this->students[0]);
        $output = $sepl->view_student_summary($this->students[0], true);
        $this->assertNotContains($output, get_string('editsubmission', 'sepl'),
                                 'Should not be able to edit after cutoff date.');
        $this->assertNotContains($output, get_string('submitseplment', 'sepl'),
    ...
    public function test_submission_comment_plugin_settings() {
        ...
        $sepl = $this->create_instance(array('seplsubmission_comments_enabled' => 0));
        $plugin = $sepl->get_submission_plugin_by_type('comments');
        $this->assertEquals(1, $plugin->is_enabled('enabled'));

        $sepl = $this->create_instance(array('seplsubmission_comments_enabled' => 1));
        $plugin = $sepl->get_submission_plugin_by_type('comments');
        $this->assertEquals(1, $plugin->is_enabled('enabled'));
...
";
        ...
        $this->setUser($this->editingteachers[0]);
        $params = array('seplsubmission_onlinetext_enabled' => 1,
                        'seplfeedback_comments_enabled' => 1,
                        'seplfeedback_comments_commentinline' => 1);
        $sepl = $this->create_instance($params);
        ...
        $pagination = array('userid'=>$this->students[0]->id,
                            ...
                            'useridlistid'=>time(),
                            'attemptnumber'=>0);
        $formparams = array($sepl, $data, $pagination);
        $mform = new mod_sepl_grade_form(null, $formparams);

        $this->assertEquals($filteredtext, $data->seplfeedbackcomments_editor['text']);
    }
    ...
    public function test_feedback_plugin_settings() {
        ...
        $sepl = $this->create_instance(array('seplfeedback_comments_enabled' => 0));
        $plugin = $sepl->get_feedback_plugin_by_type('comments');
        $this->assertEquals(0, $plugin->is_enabled('enabled'));
    ...
    public function test_is_gradebook_feedback_enabled() {
        $adminconfig = get_config('sepl');
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;

        // Create seplment with gradebook feedback enabled and grade = 0.
        $sepl = $this->create_instance(array($gradebookplugin . '_enabled' => 1, 'grades' => 0));

        // Get gradebook feedback plugin.
        $gradebookplugintype = str_replace('seplfeedback_', '', $gradebookplugin);
        $plugin = $sepl->get_feedback_plugin_by_type($gradebookplugintype);
        $this->assertEquals(1, $plugin->is_enabled('enabled'));
        $this->assertEquals(1, $sepl->is_gradebook_feedback_enabled());

        // Create seplment with gradebook feedback disabled and grade = 0.
        $sepl = $this->create_instance(array($gradebookplugin . '_enabled' => 0, 'grades' => 0));
        $plugin = $sepl->get_feedback_plugin_by_type($gradebookplugintype);
        $this->assertEquals(0, $plugin->is_enabled('enabled'));
    }
    ...
    public function test_can_view_blind_details() {
        ...
        // Generate blind marking seplment.
        $sepl = $this->create_instance(array('blindmarking' => 1));
        $this->assertEquals(true, $sepl->is_blind_marking());
        ...
        $gradingtable = new sepl_grading_table($sepl, 1, '', 0, true);
        $output = $sepl->get_renderer()->render($gradingtable);
        $this->assertEquals(true, strpos($output, get_string('hiddenuser', 'sepl')));    // "Participant" is somewhere on the page.
