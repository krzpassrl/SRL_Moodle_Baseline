...
/**
 * Define all the restore steps that will be used by the restore_sepl_activity_task
 *
 * @package   mod_sepl
...
/**
 * Define the complete seplment structure for restore, with file and id annotations
 *
 * @package   mod_sepl
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_sepl_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('sepl', '/activity/sepl');
        if ($userinfo) {
            $submission = new restore_path_element('sepl_submission',
                                                   '/activity/sepl/submissions/submission');
            $paths[] = $submission;
            $this->add_subplugin_structure('seplsubmission', $submission);
            $grade = new restore_path_element('sepl_grade', '/activity/sepl/grades/grade');
            $paths[] = $grade;
            $this->add_subplugin_structure('seplfeedback', $grade);
            $userflag = new restore_path_element('sepl_userflag',
                                                   '/activity/sepl/userflags/userflag');
            $paths[] = $userflag;
        }
        $paths[] = new restore_path_element('sepl_plugin_config',
                                            '/activity/sepl/plugin_configs/plugin_config');

        return $this->prepare_activity_structure($paths);
    }

    /**
     * Process an sepl restore.
     *
     * @param object $data The data in object form
     * @return void
     */
    protected function process_sepl($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('sepl', $data);
     ...
     * @return void
     */
    protected function process_sepl_submission($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('sepl_submission', $data);
     ...
     * @return void
     */
    protected function process_sepl_userflag($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('sepl_user_flags', $data);
    }
     ...
     * @return void
     */
    protected function process_sepl_grade($data) {
        global $DB;
        ...
        if (!empty($data->extensionduedate) ||
                ...
                !empty($data->locked)) {
            $flags = new stdClass();
            $flags->seplment = $this->get_new_parentid('sepl');
            if (!empty($data->extensionduedate)) {
        ...
        $newitemid = $DB->insert_record('sepl_grades', $data);
     ...
     * @return void
     */
    protected function process_sepl_plugin_config($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('sepl_plugin_config', $data);
    }

    /**
     * For all submissions in this seplment, either set the
    ...
    protected function set_latest_submission_field() {
        ...
        $seplmentid = $this->get_new_parentid('sepl');
        ...
        $sql = 'SELECT DISTINCT userid FROM {sepl_submission} WHERE seplment = ? AND groupid = ?';
        $params = array($seplmentid, 0);
        $users = $DB->get_records_sql($sql, $params);

        foreach ($users as $userid => $unused) {
            $params = array('seplment'=>$seplmentid, 'groupid'=>0, 'userid'=>$userid);
        ...
        $sql = 'SELECT DISTINCT groupid FROM {sepl_submission} WHERE seplment = ? AND userid = ?';
        $params = array($seplmentid, 0);
        $groups = $DB->get_records_sql($sql, $params);

        foreach ($groups as $groupid => $unused) {
            $params = array('seplment'=>$seplmentid, 'userid'=>0, 'groupid'=>$groupid);
        ...
        $records = $DB->get_recordset_sql('SELECT g.id, g.userid
                                      ...
                                      LEFT JOIN {sepl_submission} s
                                          ...
                                          WHERE s.id IS NULL AND g.seplment = ?', array($seplmentid));

        $submissions = array();
        foreach ($records as $record) {
            $submission = new stdClass();
            $submission->seplment = $seplmentid;
    ...
    protected function after_execute() {
        $this->add_related_files('mod_sepl', 'intro', null);
        $this->add_related_files('mod_sepl', 'introattachment', null);
