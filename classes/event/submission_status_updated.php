...
/**
 * The mod_sepl submission status updated event.
 *
 * @package    mod_sepl
...
/**
 * The mod_sepl submission status updated event class.
 *
...
class submission_status_updated extends base {
    ...
    public static function create_from_submission(\sepl $sepl, \stdClass $submission) {
        $data = array(
            ...
            'relateduserid' => ($sepl->get_instance()->teamsubmission) ? null : $submission->userid,
            'other' => array(
    ...
    public function get_description() {
        return "The user with id '$this->userid' has updated the status of the submission with id '$this->objectid' for " .
            "the seplment with course module id '$this->contextinstanceid' to the status '{$this->other['newstatus']}'.";
    }
    ...
    public static function get_name() {
        return get_string('eventsubmissionstatusupdated', 'mod_sepl');
    }
    ...
    protected function get_legacy_logdata() {
        ...
        $logmessage = get_string('reverttodraftforstudent', 'sepl', array('id' => $user->id, 'fullname' => fullname($user)));
