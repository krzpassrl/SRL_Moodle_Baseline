 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sepl_grading_batch_operations_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        // Visible elements.
        $options = array();
        $options['lock'] = get_string('locksubmissions', 'sepl');
        $options['unlock'] = get_string('unlocksubmissions', 'sepl');
        if ($instance['submissiondrafts']) {
            $options['reverttodraft'] = get_string('reverttodraft', 'sepl');
        }
        if ($instance['duedate'] && has_capability('mod/sepl:grantextension', $instance['context'])) {
            $options['grantextension'] = get_string('grantextension', 'sepl');
        }
        if ($instance['attemptreopenmethod'] == ASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL) {
            $options['addattempt'] = get_string('addattempt', 'sepl');
        }
        ...
        if ($instance['markingworkflow']) {
            $options['setmarkingworkflowstate'] = get_string('setmarkingworkflowstate', 'sepl');
        }
        if ($instance['markingallocation']) {
            $options['setmarkingallocation'] = get_string('setmarkingallocation', 'sepl');
        }
        ...
        $objs = array();
        $objs[] =& $mform->createElement('select', 'operation', get_string('chooseoperation', 'sepl'), $options);
        $objs[] =& $mform->createElement('submit', 'submit', get_string('go'));
        $batchdescription = get_string('batchoperationsdescription', 'sepl');
