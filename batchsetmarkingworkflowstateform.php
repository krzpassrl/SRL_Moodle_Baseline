...
require_once($CFG->dirroot . '/mod/sepl/feedback/file/locallib.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sepl_batch_set_marking_workflow_state_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        $formheader = get_string('batchsetmarkingworkflowstateforusers', 'sepl', $params['userscount']);

        $mform->addElement('header', 'general', $formheader);
        $mform->addElement('static', 'userslist', get_string('selectedusers', 'sepl'), $params['usershtml']);

        $options = $params['markingworkflowstates'];
        $mform->addElement('select', 'markingworkflowstate', get_string('markingworkflowstate', 'sepl'), $options);
